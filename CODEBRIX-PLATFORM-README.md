...
An optional add-on package providing audio playback (WAV, MP3, Ogg Vorbis and FLAC) and MIDI music with a single reference in the application's core library and no native setup at all — unlike the WebView and MediaPlayer add-ons there is no per-OS engine and nothing to install, so the package is live on all six platform heads, macOS included. It provides three public types plus the enumeration one of them reports with: `AudioPlayer`, a non-visual XAML-declarable element with `Play`/`Pause`/`Stop`/`Seek` control and bindable `Source`, `Volume`, `IsLooping`, `Duration`, and `Position` properties — the position is two-way bindable and debounced, so binding a `Slider` to it yields a working scrubber with clean seek-on-release behavior; `MidiPlayer`, which renders a MIDI file through a SoundFont (`.sf2`), an SFZ (`.sfz`) or a Decent Sampler (`.dspreset`, `.dslibrary`, `.dsbundle`, or a folder holding a preset) instrument and carries that same transport surface — so the same scrubber markup drives either player — plus `Speed` (tempo without pitch change), a live voice count, a beat clock (`BeatsPerMinute` and `BeatPosition`), per-channel mixing, and an observe-only hook for reacting to the notes; and `SoundEffect`, a fire-and-forget one-liner for overlapping sound effects with in-memory caching. `MidiInstrumentKind` reports which of the three instrument formats actually loaded. A Decent Sampler preset carries its own knobs, sliders and menus, and `MidiPlayer` surfaces them as live parameters (`InstrumentControls`, `InstrumentTagStates`, `GetInstrumentControl`, and an `InstrumentControlChanged` event raised on the UI thread and coalesced per dispatcher pass, because the preset's own modulators can move a knob on every rendered audio block) — the seam a page renders its own controls over, since nothing here draws a preset's interface. The MIDI Polyphonic Expression settings a recorded performance needs are surfaced too (`MpeMode`, `MpeMemberBendRange`, the per-zone member counts, the read-only zone information and `GetReleaseVelocity`), as are `InstrumentPreset` for a container holding several presets, `InstrumentLoadOptions` for the memory and streaming policy, `DropAuxiliaryOutputs`, and what the load reported: `InstrumentProblems`, `UnsupportedInstrumentOpcodes`, `UnsupportedInstrumentFeatures`, `InstrumentMemorySummary` and `SourceProblems` — the last being anything the MIDI file itself needed forgiving, since a file that breaks a rule is read leniently rather than refused. Sources accept a filesystem path, an `ms-appx:///` asset URI, an `embedded://` embedded-resource URI, or a stream — with one exception: an `.sfz` instrument and every Decent Sampler form must be a path or `ms-appx:///` URI, because each of them needs a real place on disk (an `.sfz` and a `.dspreset` reference their sample files as separate files beside them, and a `.dslibrary` or `.dsbundle`, though it is one file, is read in place from the archive rather than unpacked). Opus plays as well, once the application references the separate `CodeBrix.Audio.Opus.BsdLicenseForever` package and registers it at start-up, and the oscillators and creative effects a Decent Sampler preset may ask for arrive the same way through `CodeBrix.Audio.ModestSynth.MitLicenseForever`; this add-on neither depends on either nor needs to — without the second, such a preset still loads and every sample group still plays, and what it could not have is named in the reporting properties. Because instruments can be very large, `MidiPlayer` loads them in the background and reports progress through `IsLoading` and `MediaOpened`. The code for this package is original to CodeBrix, with playback delivered through the fully managed `CodeBrix.Audio.MitLicenseForever` package and its bundled native audio backend (see Section 5.3).
