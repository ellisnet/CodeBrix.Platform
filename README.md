...
| `CodeBrix.Platform.AudioPlayer.ApacheLicenseForever` | Audio playback (`AudioPlayer` element + `SoundEffect`) and MIDI music through a SoundFont, SFZ or Decent Sampler instrument (`MidiPlayer` element — with the preset's own control model, the MPE settings and a beat clock), fully managed on all heads |
