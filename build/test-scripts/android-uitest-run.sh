...
then
	export TEST_FILTERS="\
		...
		& Category~testBucket:$CODEBRIX_UITEST_BUCKET_ID\
	";
...
export CODEBRIX_ANDROID_BUILD_TOOLS_VERSION="35.0.1"
export CODEBRIX_UITEST_SCREENSHOT_PATH=$BUILD_ARTIFACTSTAGINGDIRECTORY/screenshots/$SCREENSHOTS_FOLDERNAME
export CODEBRIX_UITEST_APP_ID="uno.platform.unosampleapp"
export CODEBRIX_UITEST_PLATFORM=Android
export CODEBRIX_UITEST_ANDROIDAPK_PATH=$BUILD_SOURCESDIRECTORY/build/$SAMPLEAPP_ARTIFACT_NAME/android/uno.platform.unosampleapp-Signed.apk
...
export CODEBRIX_TESTS_FAILED_LIST=$BUILD_SOURCESDIRECTORY/build/uitests-failure-results/failed-tests-android-$ANDROID_SIMULATOR_APILEVEL-$SCREENSHOTS_FOLDERNAME-$CODEBRIX_UITEST_BUCKET_ID-$UITEST_RUNTIME_TEST_GROUP-$TARGETPLATFORM_NAME.txt
export CODEBRIX_TESTS_RESPONSE_FILE=$BUILD_SOURCESDIRECTORY/build/nunit.response
export CODEBRIX_UITEST_RUNTIMETESTS_RESULTS_FILE_PATH=$BUILD_SOURCESDIRECTORY/build/RuntimeTestResults-android-automated-$ANDROID_SIMULATOR_APILEVEL-$TARGETPLATFORM_NAME.xml

mkdir -p $CODEBRIX_UITEST_SCREENSHOT_PATH

if [ -f "$CODEBRIX_TESTS_FAILED_LIST" ] && [ `cat "$CODEBRIX_TESTS_FAILED_LIST"` = "invalid-test-for-retry" ];
then
...
then
	...
	echo "y" | $ANDROID_HOME/cmdline-tools/latest/bin/sdkmanager --sdk_root=${ANDROID_HOME} --install "build-tools;$CODEBRIX_ANDROID_BUILD_TOOLS_VERSION" | tr '\r' '\n' | uniq
	...
	nohup $ANDROID_HOME/emulator/emulator \
		...
		> $BUILD_ARTIFACTSTAGINGDIRECTORY/screenshots/$SCREENSHOTS_FOLDERNAME/android-emulator-log-$CODEBRIX_UITEST_BUCKET_ID-$UITEST_TEST_MODE_NAME.txt 2>&1 &
...
cp $CODEBRIX_UITEST_ANDROIDAPK_PATH $BUILD_ARTIFACTSTAGINGDIRECTORY
...
then
	...
	# Install the app
	$ANDROID_HOME/platform-tools/adb install $CODEBRIX_UITEST_ANDROIDAPK_PATH
	...
	if [ -f "$CODEBRIX_TESTS_FAILED_LIST" ]; then
		export UITEST_RUNTIME_TESTS_FILTER=`cat $CODEBRIX_TESTS_FAILED_LIST | base64 -w 0`
		...
		if [ -n "$CODEBRIX_TESTS_FAILED_LIST" ]; then
			echo "Tests to run: $CODEBRIX_TESTS_FAILED_LIST"
		fi
	fi
	...
	$ANDROID_HOME/platform-tools/adb shell pm grant $CODEBRIX_UITEST_APP_ID android.permission.WRITE_EXTERNAL_STORAGE
	$ANDROID_HOME/platform-tools/adb shell pm grant $CODEBRIX_UITEST_APP_ID android.permission.READ_EXTERNAL_STORAGE

	# start the android app using environment variables using adb
	$ANDROID_HOME/platform-tools/adb shell monkey -p $CODEBRIX_UITEST_APP_ID -c android.intent.category.LAUNCHER 1
	...
	while [[ ! $($ANDROID_HOME/platform-tools/adb shell test -e "$UITEST_RUNTIME_AUTOSTART_RESULT_FILE" > /dev/null) && $SECONDS -lt $END_TIME ]]; do
		...
		if ! $ANDROID_HOME/platform-tools/adb shell ps | grep "$CODEBRIX_UITEST_APP_ID" > /dev/null; then
			echo "The app is not running anymore"
			break
		fi
	done

	$ANDROID_HOME/platform-tools/adb pull $UITEST_RUNTIME_AUTOSTART_RESULT_FILE $CODEBRIX_ORIGINAL_TEST_RESULTS || true
	...
	echo "--filter:\"$CODEBRIX_TESTS_FILTER\"" > tests.rsp
	...
	zip -u "$CODEBRIX_UITEST_ANDROIDAPK_PATH" assemblies.blob

	## Run NUnit tests
	dotnet run -c Release -bl:$CODEBRIX_ORIGINAL_TEST_RESULTS_DIRECTORY/android-test.binlog -- --results-directory $CODEBRIX_ORIGINAL_TEST_RESULTS_DIRECTORY --settings .runsettings @tests.rsp || true
fi

## Dump the emulator's system log
$ANDROID_HOME/platform-tools/adb shell logcat -d > $BUILD_ARTIFACTSTAGINGDIRECTORY/screenshots/$SCREENSHOTS_FOLDERNAME/android-device-log-$CODEBRIX_UITEST_BUCKET_ID-$UITEST_RUNTIME_TEST_GROUP-$UITEST_TEST_MODE_NAME.txt

if [ ! -f "$CODEBRIX_ORIGINAL_TEST_RESULTS" ]; then
	echo "ERROR: The test results file $CODEBRIX_ORIGINAL_TEST_RESULTS does not exist (did nunit crash ?)"
fi
