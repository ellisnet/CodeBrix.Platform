...
export CODEBRIX_TESTS_FAILED_LIST=$BUILD_SOURCESDIRECTORY/build/uitests-failure-results/failed-tests-skia-wasm-runtimetests-$UITEST_RUNTIME_TEST_GROUP-chromium.txt
...
if [ $? -eq 0 ]; then
	dotnet run list-failed $RESULTS_FILE $CODEBRIX_TESTS_FAILED_LIST
fi
