...
export CODEBRIX_UITEST_PLATFORM=iOS
export CODEBRIX_UITEST_SCREENSHOT_PATH=$BUILD_ARTIFACTSTAGINGDIRECTORY/screenshots/$SCREENSHOTS_FOLDERNAME
...
export CODEBRIX_TESTS_FAILED_LIST=$BUILD_SOURCESDIRECTORY/build/uitests-failure-results/failed-tests-ios-$SCREENSHOTS_FOLDERNAME-${UITEST_SNAPSHOTS_GROUP=automated}-${UITEST_AUTOMATED_GROUP=automated}-${UITEST_RUNTIME_TEST_GROUP=automated}.txt
export CODEBRIX_TESTS_RUNTIMETESTS_FAILED_LIST=$BUILD_SOURCESDIRECTORY/build/uitests-failure-results/failed-runtime-tests-ios-$SCREENSHOTS_FOLDERNAME-${UITEST_SNAPSHOTS_GROUP=automated}-${UITEST_AUTOMATED_GROUP=automated}-${UITEST_RUNTIME_TEST_GROUP=automated}.txt
...
export CODEBRIX_UITEST_BENCHMARKS_PATH=$BUILD_ARTIFACTSTAGINGDIRECTORY/benchmarks/ios-automated
export CODEBRIX_UITEST_RUNTIMETESTS_RESULTS_FILE_PATH=$BUILD_SOURCESDIRECTORY/build/RuntimeTestResults-ios-automated.xml

export CODEBRIX_UITEST_SIMULATOR_VERSION="com.apple.CoreSimulator.SimRuntime.iOS-17-5"
export CODEBRIX_UITEST_SIMULATOR_NAME="iPad Pro (12.9-inch) (6th generation)"

export UnoTargetFrameworkOverride="net9.0-ios18.0"

UITEST_IGNORE_RERUN_FILE="${UITEST_IGNORE_RERUN_FILE:=false}"

if [ -f "$CODEBRIX_TESTS_FAILED_LIST" ] && [ `cat "$CODEBRIX_TESTS_FAILED_LIST"` = "invalid-test-for-retry" ] && [ "$UITEST_IGNORE_RERUN_FILE" != "true" ]; then
...
while true; do
	export UITEST_IOSDEVICE_ID=`xcrun simctl list -j | jq -r --arg sim "$CODEBRIX_UITEST_SIMULATOR_VERSION" --arg name "$CODEBRIX_UITEST_SIMULATOR_NAME" '.devices[$sim] | .[] | select(.name==$name) | .udid'`
	export UITEST_IOSDEVICE_DATA_PATH=`xcrun simctl list -j | jq -r --arg sim "$CODEBRIX_UITEST_SIMULATOR_VERSION" --arg name "$CODEBRIX_UITEST_SIMULATOR_NAME" '.devices[$sim] | .[] | select(.name==$name) | .dataPath'`
...
##
echo "Starting simulator: [$UITEST_IOSDEVICE_ID] ($CODEBRIX_UITEST_SIMULATOR_VERSION / $CODEBRIX_UITEST_SIMULATOR_NAME)"
xcrun simctl boot "$UITEST_IOSDEVICE_ID" || true

# echo "Install app on simulator: $UITEST_IOSDEVICE_ID"
# xcrun simctl install "$UITEST_IOSDEVICE_ID" "$CODEBRIX_UITEST_IOSBUNDLE_PATH" || true
idb install --udid "$UITEST_IOSDEVICE_ID" "$CODEBRIX_UITEST_IOSBUNDLE_PATH"
...
sudo chmod -R +x $CODEBRIX_UITEST_IOSBUNDLE_PATH

# Move to the screenshot directory so that the output path is the proper one, as
# required by Xamarin.UITest
cd $CODEBRIX_UITEST_SCREENSHOT_PATH

## Build the NUnit configuration file
if [ -f "$CODEBRIX_TESTS_FAILED_LIST" ] && [ "$UITEST_IGNORE_RERUN_FILE" != "true" ]; then
...
then
	...
	# $CODEBRIX_TESTS_RUNTIMETESTS_FAILED_LIST file exists
	if [ -f "$CODEBRIX_TESTS_RUNTIMETESTS_FAILED_LIST" ]; then

		# if the  only contains `invalid-test-for-retry`, exit the script
		if [ `cat "$CODEBRIX_TESTS_RUNTIMETESTS_FAILED_LIST"` = "invalid-test-for-retry" ]; then
			echo "The file $CODEBRIX_TESTS_RUNTIMETESTS_FAILED_LIST does not contain tests to re-run, skipping."
			exit 0
		fi

		export SIMCTL_CHILD_UITEST_RUNTIME_TESTS_FILTER=`cat $CODEBRIX_TESTS_RUNTIMETESTS_FAILED_LIST | base64 -b 0`
	...
	if [ -f "$SIMCTL_CHILD_UITEST_RUNTIME_AUTOSTART_RESULT_FILE" ]; then
		...
		cp -f "$SIMCTL_CHILD_UITEST_RUNTIME_AUTOSTART_RESULT_FILE" "$CODEBRIX_ORIGINAL_TEST_RESULTS"
	else
	...
	## Run tests
	dotnet run -c Release -- --results-directory $CODEBRIX_ORIGINAL_TEST_RESULTS_DIRECTORY --hangdump --hangdump-timeout 45m --hangdump-filename hang.dump --settings .runsettings --filter "$CODEBRIX_TESTS_FILTER" || true
fi
...
cp -fv "$CODEBRIX_ORIGINAL_TEST_RESULTS" $LOG_FILEPATH/Test-Results-$LOG_PREFIX.xml || true

# Copy all the dotnet test dmp files to the log directory
find $AGENT_TEMPDIRECTORY -name "*.dmp" -exec cp -v {} $LOG_FILEPATH \;
find $CODEBRIX_TESTS_LOCAL_TESTS_FILE -name "*.dmp" -exec cp -v {} $LOG_FILEPATH \;
...
if [ ! -f "$CODEBRIX_ORIGINAL_TEST_RESULTS" ]; then
	echo "##vso[task.logissue type=error]UNOBLD003: ERROR: The test results file $CODEBRIX_ORIGINAL_TEST_RESULTS does not exist (did nunit crash ?)"
fi
...
then
	...
	if [ $? -eq 0 ]; then
		dotnet run list-failed $SIMCTL_CHILD_UITEST_RUNTIME_AUTOSTART_RESULT_FILE $CODEBRIX_TESTS_RUNTIMETESTS_FAILED_LIST
	fi
fi
