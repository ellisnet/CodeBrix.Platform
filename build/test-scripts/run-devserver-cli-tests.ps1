...
function Setup-UnoStudioLicenses {
    ...
    if ($env:CODEBRIX_STUDIO_LICENSES_BIN) {
        $licensesPath = Join-Path $unoDir 'licenses.bin'
        try {
            $env:CODEBRIX_STUDIO_LICENSES_BIN | Out-File -FilePath $licensesPath -Encoding utf8 -Force
            Write-Log "Wrote CODEBRIX_STUDIO_LICENSES_BIN to: $licensesPath"
        }
        catch {
            Write-Warning "Failed to write CODEBRIX_STUDIO_LICENSES_BIN to $licensesPath : $_"
        }
    }
    else {
        Write-Log "CODEBRIX_STUDIO_LICENSES_BIN environment variable not set. Skipping license setup."
    }

    if ($env:CODEBRIX_STUDIO_USER_JSON) {
        $userJsonPath = Join-Path $unoDir 'user.json'
        try {
            $env:CODEBRIX_STUDIO_USER_JSON | Out-File -FilePath $userJsonPath -Encoding utf8 -Force
            Write-Log "Wrote CODEBRIX_STUDIO_USER_JSON to: $userJsonPath"
        }
        catch {
            Write-Warning "Failed to write CODEBRIX_STUDIO_USER_JSON to $userJsonPath : $_"
        }
    }
    else {
        Write-Log "CODEBRIX_STUDIO_USER_JSON environment variable not set. Skipping user.json setup."
    }
