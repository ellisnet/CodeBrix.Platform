...
if ( ($TestGroup -eq 0) -and ($env:UWPBuildEnabled -eq 'True') )
{
    ...
    if ($IsWindows) 
    {
        ...
        & dotnet build -c Debug $default "UnoAppWinUI.Server\PlatformAppWinUI.Server.csproj"
        ...
        & $msbuild $debug "/p:Platform=x86" "UnoAppWinUI.Windows\PlatformAppWinUI.Windows.csproj" "/p:TargetFrameworks=net8.0-windows10.0.19041;TargetFramework=net8.0-windows10.0.19041" "/bl:../binlogs/UnoAppWinUI.Windows/debug/$i/msbuild.binlog"
        Assert-ExitCodeIsZero
    }
    ...
    # Assert-ExitCodeIsZero
    # 
    # dotnet run -c Debug --project src\Platform.XamlTrimmingValidator\Platform.XamlTrimmingValidator.csproj -- --hints-file=build\assets\MyAppXamlTrim-hints.txt --target-assembly=MyAppXamlTrim\MyAppXamlTrim.Skia.Gtk\bin\Debug\net6.0\win-x64\publish\Uno.UI.dll
    # Assert-ExitCodeIsZero
    ...
    if ($IsWindows) 
    {
        ...
        dotnet run --project ..\Platform.ResourceTrimmingValidator\Platform.ResourceTrimmingValidator.csproj -- -a (Get-ChildItem MyAppXamlTrim.Wasm.clr -Recurse).FullName -r Strings.en.Resources.upri -x Strings.fr.Resources.upri
        Assert-ExitCodeIsZero

        dotnet run --project ..\Platform.ResourceTrimmingValidator\Platform.ResourceTrimmingValidator.csproj -- -a (Get-ChildItem Uno.UI.clr -Recurse).FullName -r Resources.Strings.en.Resources.upri -r UI.Xaml.DragDrop.Strings.en-US.Resources.upri -x Resources.Strings.cs-CZ.Resources.upri
        Assert-ExitCodeIsZero
        ...
        $responseFile = @(
            ...
            "MyUnoLib2\MyPlatformLib2.csproj",
            "-bl",
            "/p:TargetFrameworks=""net8.0-windows10.0.19041;net8.0"""
        )
        $responseFile | Out-File -FilePath "build.rsp" -Encoding ASCII
...
& $env:BUILD_SOURCESDIRECTORY/build/test-scripts/update-platform-sdk-globaljson.ps1
...
@(
    ...
    @(3, "5.6/uno56droidioswasmskia/Uno56NugetLibrary/Platform56NugetLibrary.csproj", @("-p:PackageOutputPath=$env:BUILD_SOURCESDIRECTORY\src\PackageCache"), @("macOS", "NetCore", "CleanNugetTemp","NoBuildClean")),
