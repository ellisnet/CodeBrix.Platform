...
export CODEBRIX_UITEST_APP_ID="${CODEBRIX_UITEST_APP_ID=uno.platform.samplesapp.skia}"
export CODEBRIX_UITEST_ANDROIDAPK_PATH=$BUILD_SOURCESDIRECTORY/build/$SAMPLEAPP_ARTIFACT_NAME/android/$CODEBRIX_UITEST_APP_ID-Signed.apk
export UITEST_RUNTIME_TEST_GROUP=${UITEST_RUNTIME_TEST_GROUP=automated}
export CODEBRIX_ORIGINAL_TEST_RESULTS=$BUILD_SOURCESDIRECTORY/build/TestResult-original.xml
export CODEBRIX_TESTS_FAILED_LIST=$BUILD_SOURCESDIRECTORY/build/uitests-failure-results/failed-tests-skia-android-runtimetests-$UITEST_RUNTIME_TEST_GROUP.txt
...
then
	...
	nohup $ANDROID_HOME/emulator/emulator \
		...
		> $LOGS_PATH/android-emulator-log-$CODEBRIX_UITEST_BUCKET_ID-$UITEST_TEST_MODE_NAME.txt 2>&1 &
...
UITEST_RUNTIME_AUTOSTART_RESULT_DEVICE_BASE_PATH="/storage/emulated/0/Android/data/$CODEBRIX_UITEST_APP_ID/files"
...
$ANDROID_HOME/platform-tools/adb shell am start \
  -n $CODEBRIX_UITEST_APP_ID/crc6448f3b0362cbf4bc9.MainActivity \
...
while [[ ! $($ANDROID_HOME/platform-tools/adb shell test -e "$UITEST_RUNTIME_AUTOSTART_RESULT_DEVICE_PATH" > /dev/null) && $SECONDS -lt $END_TIME ]]; do
	...
	$ANDROID_HOME/platform-tools/adb shell logcat -d > $LOGS_PATH/android-device-log-$CODEBRIX_UITEST_BUCKET_ID-$UITEST_RUNTIME_TEST_GROUP-$UITEST_TEST_MODE_NAME.interim.txt
...
## Dump the emulator's system log
$ANDROID_HOME/platform-tools/adb shell logcat -d > $LOGS_PATH/android-device-log-$CODEBRIX_UITEST_BUCKET_ID-$UITEST_RUNTIME_TEST_GROUP-$UITEST_TEST_MODE_NAME.txt
...
if [ $? -eq 0 ]; then
	dotnet run list-failed $UITEST_RUNTIME_AUTOSTART_RESULT_LOCAL_PATH $CODEBRIX_TESTS_FAILED_LIST
fi
