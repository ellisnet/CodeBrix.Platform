...
namespace Windows.Globalization
{
	public partial class Calendar
	{
		...
		private DateTime DateTime => TimeZoneInfo.ConvertTime((DateTimeOffset)_time, _timeZone).DateTime;
		...
		{
			[NotImplemented]
			get => throw new global::System.NotImplementedException("The member string Calendar.NumeralSystem is not implemented in CodeBrix.");
			[NotImplemented]
		...
		public int Period
		{
			get => _clock == ClockIdentifiers.TwentyFourHour || DateTime.Hour < 12 ? 1 : 2;
			set
			{
		...
		// Apply `mutator` to the wall-clock DateTime projected through `_timeZone`, then re-anchor
		// back to `_time` preserving the same offset. Without this, the implicit DateTime ->
		// WindowsFoundationDateTime conversion treats the result as local time and shifts the
		// stored UTC instant by `_timeZone`'s offset on every Add* call.
		private void MutateTime(Func<DateTime, DateTime> mutator)
		{
			var local = TimeZoneInfo.ConvertTime((DateTimeOffset)_time, _timeZone);
			var next = mutator(local.DateTime);
			_time = new DateTimeOffset(next, local.Offset);
		}

		public void AddYears(int years)
			=> MutateTime(d => _calendar.AddYears(d, years));

		public void AddMonths(int months)
			=> MutateTime(d => _calendar.AddMonths(d, months));

		public void AddWeeks(int weeks)
			=> MutateTime(d => _calendar.AddWeeks(d, weeks));

		public void AddDays(int days)
			=> MutateTime(d => _calendar.AddDays(d, days));

		public void AddPeriods(int periods)
			=> AddHours((_clock == ClockIdentifiers.TwentyFourHour ? 24 : 12) * periods);

		public void AddHours(int hours)
			=> MutateTime(d => _calendar.AddHours(d, hours));

		public void AddMinutes(int minutes)
			=> MutateTime(d => _calendar.AddMinutes(d, minutes));

		public void AddSeconds(int seconds)
			=> MutateTime(d => _calendar.AddSeconds(d, seconds));

		public void AddNanoseconds(int nanoseconds)
			=> MutateTime(d => _calendar.AddMilliseconds(d, nanoseconds / 1000d));
		#endregion
		...
		public string EraAsString()
			=> throw new global::System.NotImplementedException("The member string Calendar.EraAsString() is not implemented in CodeBrix.");

		[NotImplemented]
		public string EraAsString(int idealLength)
			=> throw new global::System.NotImplementedException("The member string Calendar.EraAsString(int idealLength) is not implemented in CodeBrix.");

		public string YearAsString()
			=> DateTime.Year.ToString(_resolvedCulture);

		public string YearAsTruncatedString(int remainingDigits)
			=> DateTime.ToString("yy", _resolvedCulture);

		public string YearAsPaddedString(int minDigits)
			=> DateTime.Year.ToString(new string('0', minDigits), _resolvedCulture);

		internal string MonthAsFullString()
			=> MonthAsString();

		public string MonthAsString()
			=> DateTime.ToString("MMM", _resolvedCulture);
		...
		public string MonthAsNumericString()
			=> DateTime.Month.ToString(_resolvedCulture);

		public string MonthAsPaddedNumericString(int minDigits)
			=> DateTime.Month.ToString(new string('0', minDigits), _resolvedCulture);
		...
		public string PeriodAsString()
			=> DateTime.ToString("tt", _resolvedCulture);
		...
		public string HourAsPaddedString(int minDigits) =>
			_clock == ClockIdentifiers.TwentyFourHour
				? DateTime.ToString(minDigits < 2 ? "H" : "HH", _resolvedCulture)
				: DateTime.ToString(minDigits < 2 ? "h" : "hh", _resolvedCulture);

		public string MinuteAsString()
			=> DateTime.Minute.ToString(_resolvedCulture);

		public string MinuteAsPaddedString(int minDigits)
			=> DateTime.Minute.ToString(new string('0', minDigits), _resolvedCulture);

		public string SecondAsString()
			=> DateTime.Second.ToString(_resolvedCulture);

		public string SecondAsPaddedString(int minDigits)
			=> DateTime.Second.ToString(new string('0', minDigits), _resolvedCulture);

		public string NanosecondAsString()
			=> (DateTime.Millisecond * 1000).ToString(_resolvedCulture);

		public string NanosecondAsPaddedString(int minDigits)
			=> (DateTime.Millisecond * 1000).ToString(new string('0', minDigits), _resolvedCulture);
