...
internal class CodeBrixWpfWindow : WpfWindow
{
	...
	public CodeBrixWpfWindow(WinUI.Window winUIWindow, WinUI.XamlRoot xamlRoot)
	{
		...
		var windowHost = new CodeBrixWpfWindowHost(this, winUIWindow);
		Host = windowHost;
		...
		Closed += CodeBrixWpfWindow_Closed;
		Activated += CodeBrixWpfWindow_Activated;
	...
	private void CodeBrixWpfWindow_Activated(object? sender, EventArgs e)
	{
		Host.Focus();
		if (!_shown)
		{
	...
	internal static event EventHandler<CodeBrixWpfWindow>? NativeWindowShown;

	private void CodeBrixWpfWindow_Closed(object? sender, EventArgs e)
	{
		CoreApplication.GetCurrentView().TitleBar.ExtendViewIntoTitleBarChanged -= UpdateWindowPropertiesFromCoreApplication;
