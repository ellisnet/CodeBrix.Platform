	...
	internal class WpfDragDropExtension : IDragDropExtension
	{
		...
		private static DragUI? CreateDragUIForExternalDrag(IDataObject src)
		{
			...
			if (src.GetData(DataFormats.Bitmap) is BitmapSource bitmap)
			{
				...
				var codebrixImage = ConvertBitmapSourceToCodeBrixBitmapImage(bitmap);
				if (codebrixImage is not null)
				{
					dragUI.SetContentFromExternalBitmapImage(codebrixImage);
					return dragUI;
				}
			}
			...
			if (src.GetData(DataFormats.FileDrop) is string[] files && files.Length > 0)
			{
				...
				if (imageFile is not null)
				{
					try
					{
						var codebrixImage = LoadImageFromFile(imageFile);
						if (codebrixImage is not null)
						{
							dragUI.SetContentFromExternalBitmapImage(codebrixImage);
							return dragUI;
		...
		private static Microsoft.UI.Xaml.Media.Imaging.BitmapImage? LoadImageFromFile(string filePath)
		{
			try
			{
				...
				return ConvertBitmapSourceToCodeBrixBitmapImage(wpfBitmap);
			}
			catch (Exception ex) when (ex is IOException or UnauthorizedAccessException or NotSupportedException or UriFormatException)
			{
		...
		private static Microsoft.UI.Xaml.Media.Imaging.BitmapImage? ConvertBitmapSourceToCodeBrixBitmapImage(BitmapSource wpfBitmap)
		{
			try
			{
				...
				var codebrixBitmap = new Microsoft.UI.Xaml.Media.Imaging.BitmapImage();
				codebrixBitmap.SetSource(memoryStream.AsRandomAccessStream());

				return codebrixBitmap;
			}
