...
internal class WpfHostBuilder : IPlatformHostBuilder, IWindowsSkiaHostBuilder
{
	...
	private void RegisterAssemblyResolver(bool useSharedFramework)
	{
		...
		if (Uri.TryCreate(typeof(object).Assembly.Location, UriKind.RelativeOrAbsolute, out var uri))
		{
			...
			{
				throw new InvalidOperationException($"Unable to find the Microsoft.WindowsDesktop.App framework. An CodeBrix Platform update may be required (Expected path {_windowsDesktopFrameworkPath})");
			}
	...
	private static void LoadNativeImage(string image)
	{
		if (!NativeLibrary.TryLoad(
					...
					out _))
		{
			throw new InvalidOperationException($"Failed to load native image. An CodeBrix Platform update may be required. (Tried loading {image})");
		}
	}
