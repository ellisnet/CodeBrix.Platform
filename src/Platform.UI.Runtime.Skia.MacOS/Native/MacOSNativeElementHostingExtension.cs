...
internal class MacOSNativeElement : Microsoft.UI.Xaml.FrameworkElement
{
	public MacOSNativeElement()
	{
		Unloaded += (s, e) =>
		{
			NativeCodeBrix.codebrix_native_dispose(NativeHandle);
		};
	}
...
internal class MacOSNativeElementHostingExtension : ContentPresenter.INativeElementHostingExtension
{
	...
	public void ArrangeNativeElement(object content, Rect arrangeRect)
	{
		if (content is MacOSNativeElement element)
		{
			...
			{
				NativeCodeBrix.codebrix_native_arrange(element.NativeHandle, arrangeRect.Left, arrangeRect.Top, arrangeRect.Width, arrangeRect.Height);
			}
		}
	...
	public void AttachNativeElement(object content)
	{
		if (content is MacOSNativeElement element)
		{
			NativeCodeBrix.codebrix_native_attach(element.NativeHandle);
	...
	public void ChangeNativeElementOpacity(object content, double opacity)
	{
		if (content is MacOSNativeElement element)
		{
			...
			NativeCodeBrix.codebrix_native_set_opacity(element.NativeHandle, opacity);
		}
		else if (this.Log().IsEnabled(LogLevel.Debug))
		{
			this.Log().Debug($"Object `{nameof(content)}` is a {content.GetType().FullName} and not a MacOSNativeElement subclass.");
	...
	public object? CreateSampleComponent(string text)
	{
		...
		var handle = NativeCodeBrix.codebrix_native_create_sample(_window.Handle, text);
	...
	public void DetachNativeElement(object content)
	{
		if (content is MacOSNativeElement element)
		{
			...
			{
				NativeCodeBrix.codebrix_native_detach(element.NativeHandle);
	...
	public bool IsNativeElementAttached(object owner, object nativeElement)
	{
		...
		{
			return NativeCodeBrix.codebrix_native_is_attached(element.NativeHandle);
		}
		else if (this.Log().IsEnabled(LogLevel.Debug))
		{
	...
	public Size MeasureNativeElement(object content, Size childMeasuredSize, Size availableSize)
	{
		if (content is MacOSNativeElement element)
		{
			NativeCodeBrix.codebrix_native_measure(element.NativeHandle, childMeasuredSize.Width, childMeasuredSize.Height, availableSize.Width, availableSize.Height, out var width, out var height);
