...
public class MacSkiaHost : SkiaHost, ISkiaApplicationHost
{
	...
	protected override unsafe Task RunLoop()
	{
		...
		NativeCodeBrix.codebrix_set_application_start_callback(&StartApp);
	...
	static private void StartApp()
	{
		try
		{
			...
			if (NativeCodeBrix.codebrix_application_is_bundled())
			{
	...
	private bool InitializeMac()
	{
		try
		{
			...
			var result = NativeCodeBrix.codebrix_app_initialize(ref metal);
