...
internal class MacOSMediaPlayerExtension : IMediaPlayerExtension
{
	...
	private MacOSMediaPlayerExtension(object owner)
	{
		...
		{
			_player = player;
			_nativePlayer = NativeCodeBrix.codebrix_mediaplayer_create();
			...
			NativeCodeBrix.codebrix_mediaplayer_set_notifications(_nativePlayer);
		}
		else
		{
	...
	{
		NativeCodeBrix.codebrix_mediaplayer_set_callbacks(
	...
	{
		get => NativeCodeBrix.codebrix_mediaplayer_get_rate(_nativePlayer);
		set => NativeCodeBrix.codebrix_mediaplayer_set_rate(_nativePlayer, (float)value);
	}
	...
	{
		get { return TimeSpan.FromSeconds(NativeCodeBrix.codebrix_mediaplayer_get_current_time(_nativePlayer)); }
		set { NativeCodeBrix.codebrix_mediaplayer_set_current_time(_nativePlayer, value.TotalSeconds); }
	}

	public bool? IsVideo => NativeCodeBrix.codebrix_mediaplayer_is_video(_nativePlayer);
	...
	private Uri? Uri
	{
		...
		set
		{
			...
			if (uri.IsLocalResource())
			{
				...
				var baseUrl = NativeCodeBrix.codebrix_application_is_bundled() ? "[ResourcePath]" : Windows.ApplicationModel.Package.Current.InstalledPath;
				NativeCodeBrix.codebrix_mediaplayer_set_source_path(_nativePlayer, Path.Combine(baseUrl, filePath.TrimStart('/')));
			}
			else if (uri.IsAppData())
			{
				NativeCodeBrix.codebrix_mediaplayer_set_source_path(_nativePlayer, AppDataUriEvaluator.ToPath(uri));
			}
			else
			{
				NativeCodeBrix.codebrix_mediaplayer_set_source_uri(_nativePlayer, _uri.ToString());
			}
	...
	{
		NativeCodeBrix.codebrix_mediaplayer_set_volume(_nativePlayer, (float)_player.Volume);
	}
	...
	public void Pause()
	{
		...
		{
			NativeCodeBrix.codebrix_mediaplayer_pause(_nativePlayer);
	...
	public void Play()
	{
		...
		try
		{
			...
			NativeCodeBrix.codebrix_mediaplayer_play(_nativePlayer);
	...
	public void StepBackwardOneFrame() => NativeCodeBrix.codebrix_mediaplayer_step_by(_nativePlayer, -1);

	public void StepForwardOneFrame() => NativeCodeBrix.codebrix_mediaplayer_step_by(_nativePlayer, 1);

	public void Stop()
	{
		NativeCodeBrix.codebrix_mediaplayer_stop(_nativePlayer);
		_player.PlaybackSession.PlaybackState = MediaPlaybackState.None;
	}

	public void ToggleMute() => NativeCodeBrix.codebrix_mediaplayer_toggle_mute(_nativePlayer);
		...
		if (player is not null)
		{
			...
			if (session.PlaybackState is MediaPlaybackState.Opening or MediaPlaybackState.Buffering)
			{
				...
				else
				{
					...
					NativeCodeBrix.codebrix_mediaplayer_play(player._nativePlayer);
				}
			}
		}
	}
