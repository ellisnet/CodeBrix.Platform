...
internal class MacOSNativeWebView : MacOSNativeElement, INativeWebView
{
	...
	public MacOSNativeWebView(MacOSWindowNative window, CoreWebView2 owner)
	{
		...
		// logic adapted from uno/src/Uno.UI/UI/Xaml/Controls/WebView/Native/iOSmacOS/CodeBrixWKWebView.iOSmacOS.cs
		...
		_webview = NativeCodeBrix.codebrix_webview_create(_window.Handle, OkString, CancelString);
	...
	public string DocumentTitle => NativeCodeBrix.codebrix_webview_get_title(_webview);
	...
	public async Task<string?> ExecuteScriptAsync(string script, CancellationToken token)
	{
		...
		using (token.Register(() => tcs.TrySetCanceled()))
		{
			...
			NativeCodeBrix.codebrix_webview_execute_script(_webview, GCHandle.ToIntPtr(handle), script);
		}
	...
	public void GoBack() => NativeCodeBrix.codebrix_webview_go_back(_webview);

	public void GoForward() => NativeCodeBrix.codebrix_webview_go_forward(_webview);
	...
	public async Task<string?> InvokeScriptAsync(string script, string[]? arguments, CancellationToken token)
	{
		...
		{
			var handle = GCHandle.Alloc(tcs);
			NativeCodeBrix.codebrix_webview_invoke_script(_webview, GCHandle.ToIntPtr(handle), javascript);
			return await tcs.Task;
		}
	}
	...
	public void ProcessNavigation(Uri uri)
	{
		...
		{
			var baseUrl = NativeCodeBrix.codebrix_application_is_bundled() ? "[BundlePath]" : AppDomain.CurrentDomain.BaseDirectory;
		...
		else if (_owner.HostToFolderMap.TryGetValue(uri.Host.ToLowerInvariant(), out var folderName))
		{
			...
			var baseUrl = NativeCodeBrix.codebrix_application_is_bundled() ? "[ResourcePath]" : AppDomain.CurrentDomain.BaseDirectory;
		...
		NativeCodeBrix.codebrix_webview_navigate(_webview, url, null);
	}
	...
	public void ProcessNavigation(string html)
	{
		...
		NativeCodeBrix.codebrix_webview_load_html(_webview, html);
	}
	...
	public void ProcessNavigation(HttpRequestMessage httpRequestMessage)
	{
		...
		if (url is not null)
		{
			...
			NativeCodeBrix.codebrix_webview_navigate(_webview, url, headers);
		}
	}
	...
	public void Reload()
	{
		if (_lastHtmlContent != null)
		{
			...
			NativeCodeBrix.codebrix_webview_load_html(_webview, _lastHtmlContent);
		}
		else
		{
			NativeCodeBrix.codebrix_webview_reload(_webview);
		}
	}
	...
	private void SetHistoryProperties()
	{
		_owner.SetHistoryProperties(
			NativeCodeBrix.codebrix_webview_can_go_back(_webview),
			NativeCodeBrix.codebrix_webview_can_go_forward(_webview));
	}
