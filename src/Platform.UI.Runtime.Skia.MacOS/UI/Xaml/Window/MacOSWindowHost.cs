...
internal class MacOSWindowHost : IXamlRootHost, ICodeBrixKeyboardInputSource, ICodeBrixCorePointerInputSource
{
	...
	public MacOSWindowHost(MacOSWindowNative nativeWindow, Window winUIWindow, XamlRoot xamlRoot)
	{
		...
		switch (host.RenderSurfaceType)
		{
			case RenderSurfaceType.Metal:
				NativeCodeBrix.codebrix_window_get_metal_handles(_nativeWindow.Handle, out var device, out var queue);
	...
	private void MetalDraw(double nativeWidth, double nativeHeight, nint texture)
	{
		...
		if (clip != _lastSvgClipPath)
		{
			...
			if (NativeCodeBrix.codebrix_window_clip_svg(_nativeWindow.Handle, clip))
			{
	...
	public static unsafe void Register()
	{
		...
		// `libCodeBrixNativeMac.dylib` will find it already available and won't try to load it from `@rpath/libSkiaSharp.dylib`
		NativeSkia.gr_direct_context_make_metal(0, 0);

		NativeCodeBrix.codebrix_set_drawing_callbacks(&MetalDraw, &SoftDraw, &Resize);

		NativeCodeBrix.codebrix_set_window_events_callbacks(&OnRawKeyDown, &OnRawKeyUp, &OnMouseEvent, &OnMoveEvent, &Resize);
		ApiExtensibility.Register<IXamlRootHost>(typeof(ICodeBrixKeyboardInputSource), o => (o as ICodeBrixKeyboardInputSource)!);
		ApiExtensibility.Register<IXamlRootHost>(typeof(ICodeBrixCorePointerInputSource), o => (o as ICodeBrixCorePointerInputSource)!);

		NativeCodeBrix.codebrix_set_window_close_callbacks(&WindowShouldClose, &WindowClose);

		NativeCodeBrix.codebrix_set_window_screen_change_callbacks(&ScreenChanged, &ScreenParametersChanged);
	...
	private static int OnRawKeyDown(nint handle, VirtualKey key, VirtualKeyModifiers mods, uint scanCode, ushort unicode)
	{
		try
		{
			...
			if ((key == VirtualKey.Escape) && NativeCodeBrix.codebrix_window_is_full_screen(handle))
			{
	...
	// ICodeBrixCorePointerInputSource
	...
	public CoreCursor? PointerCursor
	{
		...
		set
		{
			if (value is null)
			{
				...
				{
					NativeCodeBrix.codebrix_cursor_hide();
					_pointerCursor = null;
				}
			}
			else
			{
				...
				{
					NativeCodeBrix.codebrix_cursor_show();
				}
				_pointerCursor = value;
				if (!NativeCodeBrix.codebrix_cursor_set(_pointerCursor.Type))
				{
