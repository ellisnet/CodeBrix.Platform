...
internal class MacOSLauncherExtension : ILauncherExtension
{
	...
	public Task<bool> LaunchUriAsync(Uri uri)
	{
		...
		return Task.FromResult(NativeCodeBrix.codebrix_application_open_url(uri.AbsoluteUri));
	}

	public Task<LaunchQuerySupportStatus> QueryUriSupportAsync(Uri uri, LaunchQuerySupportType launchQuerySupportType)
	{
		var canOpenUri = !Launcher.IsSpecialUri(uri) ? NativeCodeBrix.codebrix_application_query_url_support(uri.AbsoluteUri) : CanHandleSpecialUri(uri);
	...
	private static bool HandleSettingsUri(Uri uri)
	{
		...
		return NativeCodeBrix.codebrix_application_open_url(url);
	}
}
