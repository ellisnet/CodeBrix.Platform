	...
	public class PersonPictureTests
	{
		...
		public async Task VerifyDefaultsAndBasicSetting()
		{
			await RunOnUIThread.ExecuteAsync(() =>
			{
				...
				if (ApiInformation.IsTypePresent("Windows.ApplicationModel.Contacts.Contact, CodeBrix.Platform"))
				{
		...
		{
			if (ApiInformation.IsTypePresent("Microsoft.UI.Xaml.Automation.Peers.PersonPictureAutomationPeer, CodeBrix.Platform.UI"))
			{
				await RunOnUIThread.ExecuteAsync(() =>
				{
					PersonPicture personPicture = new PersonPicture();
...
#else
			string symbolsFontName = "ms-appx:///CodeBrix.Platform.Fonts.Fluent/Fonts/uno-fluentui-assets.ttf";
#endif
			await RunOnUIThread.ExecuteAsync(() =>
			{
				personPicture = new PersonPicture();
