...
namespace CodeBrix.Platform.UI.RuntimeTests.Tests.Windows_UI_Xaml //Was previously: Uno.UI.RuntimeTests.Tests.Windows_UI_Xaml
{
	...
	public class Given_FrameworkElement_And_Leak
	{
...
#if true // Disabled https://github.com/unoplatform/uno/pull/15540
...
#endif
#if true // Disabled https://github.com/unoplatform/uno/issues/9080
...
#endif
#if __SKIA__ && HAS_CODEBRIX_WINUI // Control is currently supported on Skia targets only.
		...
		[DataRow("CodeBrix.Platform.UI.Samples.Content.UITests.ButtonTestsControl.AppBar_KeyBoard", 15)]
		[DataRow("CodeBrix.Platform.UI.Samples.Content.UITests.ButtonTestsControl.Buttons", 15)]
#endif
...
#endif
		[DataRow("CodeBrix.Platform.UI.RuntimeTests.Tests.Windows_UI_Xaml.Controls.Button_Command_Leak", 15)]
		[DataRow("CodeBrix.Platform.UI.RuntimeTests.Tests.Windows_UI_Xaml.Controls.ItemsControl_ItemsSource_Leak", 15)]
#if !WINAPPSDK // Disabled - https://github.com/unoplatform/uno/issues/7860
		[DataRow("CodeBrix.Platform.UI.RuntimeTests.Tests.Windows_UI_Xaml.Controls.ContentDialog_Leak", 15, LeakTestStyles.All, RuntimeTestPlatforms.SkiaUIKit | RuntimeTestPlatforms.NativeUIKit)]
#endif
		...
		[DataRow("CodeBrix.Platform.UI.RuntimeTests.Tests.Windows_UI_Xaml.Controls.CommandBarFlyout_Leak", 15, LeakTestStyles.All, RuntimeTestPlatforms.NativeUIKit)] // flaky on native iOS
		...
		private async Task When_Add_Remove_Inner(object controlTypeRaw, int count)
		{
...
#if TRACK_REFS
			CodeBrix.Platform.UI.DataBinding.BinderReferenceHolder.IsEnabled = true;

			CodeBrix.Platform.UI.DataBinding.BinderReferenceHolder.PurgeHolders();
			var preStats = CodeBrix.Platform.UI.DataBinding.BinderReferenceHolder.GetReferenceStats()
...
#if TRACK_REFS
			var postStats = CodeBrix.Platform.UI.DataBinding.BinderReferenceHolder.GetReferenceStats()
				.GroupBy(x => x.Item1, x => x.Item2)
				.ToDictionary(g => g.Key, g => g.Sum());
			...
			async Task MaterializeControl(Type controlType, ContentControl rootContainer)
			{
				...
				if (run0Stats is not { })
				{
					run0Stats = CodeBrix.Platform.UI.DataBinding.BinderReferenceHolder.GetReferenceStats()
						.GroupBy(x => x.Item1, x => x.Item2)
						.ToDictionary(g => g.Key, g => g.Sum());
				}

				forest.Add(CodeBrix.Platform.UI.Extensions.ViewExtensions.TreeGraph(rootContainer, DescribeView));
