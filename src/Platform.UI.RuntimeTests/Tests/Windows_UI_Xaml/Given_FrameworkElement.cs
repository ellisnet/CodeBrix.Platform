...
namespace CodeBrix.Platform.UI.RuntimeTests.Tests.Windows_UI_Xaml_Controls //Was previously: Uno.UI.RuntimeTests.Tests.Windows_UI_Xaml_Controls
{
	...
	public partial class Given_FrameworkElement
	{
		...
		public async Task When_Native_Parent_And_Measure_Infinite()
		{
			...
			var measuredHeightLogical = Math.Round(CodeBrix.Platform.UI.ViewHelper.PhysicalToLogicalPixels(outer.MeasuredHeight));
		...
		{
			var sut = new CodeBrix.Platform.UI.RuntimeTests.Tests.Windows_UI_Xaml.Controls.ButtonUserControl();

			Assert.AreEqual(
				new Uri("ms-appx:///CodeBrix.Platform.UI.RuntimeTests/Tests/Windows_UI_Xaml/Controls/ButtonUserControl.xaml"),
				sut.BaseUri);
		}
		...
		public async Task When_HasNativeChildren_Should_Measure_And_Arrange()
		{
			...
			var nativeView = new UILabel() { Text = "Hello CodeBrix Platform" };
		...
		public async Task When_Removed_After_Add_But_Before_Loaded()
		{
			...
			await CodeBrix.Platform.UI.RuntimeTests.Helpers.UITestHelper.Load(sp);
