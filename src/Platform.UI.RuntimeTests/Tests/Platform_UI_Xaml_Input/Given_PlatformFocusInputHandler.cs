#if HAS_CODEBRIX // Testing internal CodeBrixFocusInputHandler, not available on Windows
...
namespace CodeBrix.Platform.UI.RuntimeTests.Tests.Platform_UI_Xaml_Input; //Was previously: Uno.UI.RuntimeTests.Tests.Uno_UI_Xaml_Input

[TestClass]
[RunsOnUIThread]
public class Given_CodeBrixFocusInputHandler
{
	...
	public async Task Validate_DepartFocusWhenCanTabOut()
	{
		...
		var inputHandler = new CodeBrixFocusInputHandler(VisualTree.GetRootOrIslandForElement(button));
	...
	private async Task VerifyXYNavigationAsync(XYFocusKeyboardNavigationMode mode, VirtualKey key, bool shouldSucceed, int targetIndex)
	{
		...
		var inputHandler = new CodeBrixFocusInputHandler(VisualTree.GetRootOrIslandForElement(centerButton));
