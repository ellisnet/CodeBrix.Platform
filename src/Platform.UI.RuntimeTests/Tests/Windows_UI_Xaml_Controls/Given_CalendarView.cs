...
public class Given_CalendarView
{
	...
	public async Task When_ItemCornerRadius()
	{
		...
		{
			var color = CodeBrix.Platform.Foundation.WebAssemblyRuntime.InvokeJS($"document.getElementById({dayItem.HtmlId}).style[\"background-color\"]");
			if (color == "rgb(255, 0, 0)")
			{
				hasOutOfScope = true;
				var result = CodeBrix.Platform.Foundation.WebAssemblyRuntime.InvokeJS($"document.getElementById({dayItem.HtmlId}).style[\"border-radius\"]");
