...
using Disposable = CodeBrix.Platform.Extensions.Disposables.Disposable;
	...
	public class Given_ScrollViewer
	{
		...
		public async Task When_Nested_Scroll_BringIntoView()
		{
			...
			innerScrollViewer.BringIntoViewRequested += (s, e) =>
			{
				...
				Assert.AreEqual(0, e.HorizontalOffset);
				Assert.AreEqual(0, e.VerticalOffset);
#if HAS_CODEBRIX // These values differ slightly from ScrollViewer's due to the fact that our implementation is based on the newer ScrollView control
				Assert.AreEqual(item, e.OriginalSource);
		...
		public async Task When_ReversedMouseWheel_Then_ScrollInReversedDirection()
		{
			...
			CodeBrix.Platform.UI.Xaml.Controls.ScrollContentPresenter.SetIsPointerWheelReversed(sut.Presenter, isReversed: true);
			await WindowHelper.WaitForIdle();
		...
		public async Task When_ContentHasNoBackground_Then_StillTouchScrollable()
		{
			var sut = new ScrollViewer()
			{
				UpdatesMode = CodeBrix.Platform.UI.Xaml.Controls.ScrollViewerUpdatesMode.Synchronous, // Make sure the VerticalOffset is being updated without any delay
				Width = 100,
				Height = 100,
				HorizontalScrollBarVisibility = ScrollBarVisibility.Visible,
				VerticalScrollBarVisibility = ScrollBarVisibility.Visible,
		...
		public async Task When_TouchScrollDownWithInertiaOnNonScrollable_Then_ParentReceiveInertia()
		{
			...
			var sut = parent = new ScrollViewer
			{
				...
				Height = 250,
				IsScrollInertiaEnabled = true,
				UpdatesMode = CodeBrix.Platform.UI.Xaml.Controls.ScrollViewerUpdatesMode.Synchronous, // Make sure to get the VerticalOffset updated immediately while dragging
				Content = child = new ScrollViewer
				{
					Height = 500,
					Width = 100,
