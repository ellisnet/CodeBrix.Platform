...
namespace CodeBrix.Platform.UI.RuntimeTests.Tests.Windows_UI_Xaml_Controls //Was previously: Uno.UI.RuntimeTests.Tests.Windows_UI_Xaml_Controls
{
	...
	public class Given_TextBlock
	{
		...
		public async Task When_Inlines_Transitively_Change()
		{
			...
			ImageAssert.HasColorInRectangle(bitmap, new Rectangle(0, 0, bitmap.Width, bitmap.Height), ((SolidColorBrush)CodeBrix.Platform.UI.Xaml.Media.DefaultBrushes.TextForegroundBrush).Color);
		}
#endif
		...
		public async Task When_Changing_Text_Through_Inlines()
		{
			...
			await CodeBrix.Platform.UI.RuntimeTests.Helpers.UITestHelper.Load(SUT);
			var width = CodeBrix.Platform.UI.Xaml.WindowManagerInterop.GetClientViewSize(SUT.HtmlId).clientSize.Width;

			SUT.Inlines.Clear();
			SUT.Inlines.Add(new Run { Text = "Updated Text" });

			await CodeBrix.Platform.UI.RuntimeTests.Helpers.UITestHelper.WaitForIdle();

			CodeBrix.Platform.UI.Xaml.WindowManagerInterop.GetClientViewSize(SUT.HtmlId).clientSize.Width.Should().BeApproximately(width, precision: width * 0.4);
		}
#endif
		...
		[DataRow("ms-appx:///CodeBrix.Platform.UI.RuntimeTests/Assets/Fonts/Roboto-Regular.ttf")]
		...
		{
#if __SKIA__
			if (!CodeBrix.Platform.Foundation.Extensibility.ApiExtensibility.IsRegistered<CodeBrix.Platform.ApplicationModel.DataTransfer.IClipboardExtension>())
			{
		...
		public async Task When_IsTextSelectionEnabled_TouchScroll_Then_DoesNotSelectText()
		{
			...
			var root = new ScrollViewer
			{
				...
				{
					Text = Enumerable.Range(0, 4096).Select(i => $"Hello CodeBrix #{i:D4}!").JoinBy(" "),
		...
		public async Task When_Focus_Changes_Selection_Is_Not_Shown()
		{
			...
			var screenshot = await UITestHelper.ScreenShot(SUT);
			ImageAssert.HasColorInRectangle(screenshot, new Rectangle(0, 0, screenshot.Width, screenshot.Height), ((SolidColorBrush)CodeBrix.Platform.UI.Xaml.Media.DefaultBrushes.SelectionHighlightColor).Color);

			focusBtn.Focus(FocusState.Programmatic);
			await UITestHelper.WaitForIdle();
			await UITestHelper.WaitForRender();
			var screenshot2 = await UITestHelper.ScreenShot(SUT);
			ImageAssert.DoesNotHaveColorInRectangle(screenshot2, new Rectangle(0, 0, screenshot2.Width, screenshot2.Height), ((SolidColorBrush)CodeBrix.Platform.UI.Xaml.Media.DefaultBrushes.SelectionHighlightColor).Color);
		}
#endif
