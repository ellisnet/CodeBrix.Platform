...
public class Given_WebView2
{
	...
	public async Task When_LocalFolder_File()
	{
		async Task Do()
		{
			...
			webView.CoreWebView2.Navigate("http://CodeBrixNativeAssets/index.html");
			await TestServices.WindowHelper.WaitFor(() => navigated, 3000);
		...
		<script type="text/javascript">
			function sendWebMessage(){
				try{
					...
					} else if (window.hasOwnProperty("codebrixWebView")) {
						// Android
						codebrixWebView.postMessage(JSON.stringify(message));
					} else if (window.hasOwnProperty("webkit") && typeof webkit.messageHandlers !== undefined) {
						// iOS and macOS
						webkit.messageHandlers.codebrixWebView.postMessage(JSON.stringify(message));
					}
				}
				catch (ex){
