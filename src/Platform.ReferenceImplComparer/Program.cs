...
namespace CodeBrix.Platform.ReferenceImplComparer //Was previously: Uno.ReferenceImplComparer
{
	class Program
	{
		static int Main(string[] args)
		{
			...
			foreach (var targetFramework in referenceTargetFrameworks)
			{
				foreach (var assembly in Directory.GetFiles(Path.Combine(filePath, "lib", targetFramework), "*.dll"))
				{
					...
					foreach (var runtimeAssembly in Directory.GetFiles(Path.Combine(filePath, "codebrix-platform-runtime", targetFramework), Path.GetFileName(assembly), SearchOption.AllDirectories))
					{
