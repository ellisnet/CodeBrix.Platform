...
public static class UriExtensions
{
	...
	public static string EscapeDataString(string value)
	{
		...
		for (var i = 0; i <= loops; i++)
		{
			if (i < loops)
			{
				sb.Append(Uri.EscapeDataString(value.AsSpan(EscapeDataStringCharactersMaxLength * i, EscapeDataStringCharactersMaxLength)));
			}
			else
			{
				sb.Append(Uri.EscapeDataString(value.AsSpan(EscapeDataStringCharactersMaxLength * i)));
			}
		}
