...
public partial class ApiInformation
{
	...
	private static bool IsImplementedByCodeBrix(MemberInfo? member) => (member?.GetCustomAttributes(typeof(CodeBrix.Platform.NotImplementedAttribute), false)?.Length ?? -1) == 0;
			...
			string typeName)
	{
		lock (_gate)
		{
			...
			{
				_isTypePresent[typeName] = result = IsImplementedByCodeBrix(GetValidType(typeName));
			}
	...
	internal static bool IsMethodPresent(Type type, string methodName)
		=> IsImplementedByCodeBrix(type?.GetMethod(methodName));
	...
	internal static bool IsEventPresent(
			...
			Type type,
			string methodName)
		=> IsImplementedByCodeBrix(type?.GetEvent(methodName));
	...
	internal static bool IsPropertyPresent(Type type, string methodName)
		=> IsImplementedByCodeBrix(type?.GetProperty(methodName));
			...
			string propertyName)
	{
		...
		if (IsImplementedByCodeBrix(property))
		{
			...
			string typeName)
	{
		lock (_assemblies)
		{
			...
			[UnconditionalSuppressMessage("Trimming", "IL2068", Justification = "`CodeBrix.Platform.UI.SourceGenerators/BindableTypeProviders` et al should ensure required members are preserved.")]
			[return: DynamicallyAccessedMembers(PublicMembers)]
			...
			[UnconditionalSuppressMessage("Trimming", "IL2073", Justification = "`CodeBrix.Platform.UI.SourceGenerators/BindableTypeProviders` et al should ensure required members are preserved.")]
			[return: DynamicallyAccessedMembers(PublicMembers)]
	...
	{
		var message = $"The member {type}.{memberName} is not implemented. For more information, visit https://github.com/ellisnet/CodeBrix.Platform/blob/main/NOT-IMPLEMENTED.md";
