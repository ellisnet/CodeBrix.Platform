...
using CodeBrix.Platform.UWPSyncGenerator.Helpers;
...
namespace CodeBrix.Platform.UWPSyncGenerator //Was previously: Uno.UWPSyncGenerator
{
	abstract class Generator
	{
		...
		private static readonly string[] _codebrixUINamespaces = new[]
		{
			"Windows.UI.Xaml",
		...
		public virtual async Task Build(string baseName, string sourceAssembly)
		{
			...
			if (baseName == "CodeBrix.Platform.UI.Dispatching")
			{
				includeNamespaces.Add("Windows.UI.Dispatching");
			}
			else if (baseName == "CodeBrix.Platform.UI.Composition")
			{
			...
			var q = from asm in origins
					...
					where ((baseName == "CodeBrix" || baseName == "CodeBrix.Platform.Foundation") && !targetType.ContainingNamespace.ToString().StartsWith("Windows.UI.Xaml", StringComparison.Ordinal) && !targetType.ContainingNamespace.ToString().StartsWith("Microsoft.UI.Xaml", StringComparison.Ordinal))
					|| (
						(baseName == "CodeBrix.Platform.UI" || baseName == "CodeBrix.Platform.UI.Dispatching" || baseName == "CodeBrix.Platform.UI.Composition")
						&& _codebrixUINamespaces.Any(n => targetType.ContainingNamespace.ToString().StartsWith(n, StringComparison.Ordinal))
					)
		...
		protected class PlatformSymbols<T> where T : ISymbol
		{
			...
			private static bool IsNotDefinedByCodeBrix(ISymbol symbol)
			{
			...
			private static bool IsImplemented(ISymbol symbol)
			{
				...
				if (IsNotDefinedByCodeBrix(symbol)) { return false; }
		...
		)
		{
			...
			foreach (var method in ifaceSymbol.GetMembers().OfType<IMethodSymbol>())
			{
				...
				if (allMethods.HasUndefined)
				{
					...
					b.AppendLineInvariant($"[global::CodeBrix.Platform.NotImplemented({allMethods.GenerateNotImplementedList()})]");
			...
			foreach (var property in ifaceSymbol.GetMembers().OfType<IPropertySymbol>())
			{
				...
				if (allProperties.HasUndefined)
				{
					...
					b.AppendLineInvariant($"[global::CodeBrix.Platform.NotImplemented({allProperties.GenerateNotImplementedList()})]");
			...
			foreach (var eventMember in type.GetMembers().OfType<IEventSymbol>())
			{
				...
				if (allMembers.HasUndefined)
				{
					...
					else
					{
						...
						b.AppendLineInvariant($"[global::CodeBrix.Platform.NotImplemented({notImplementedList})]");
		...
		private void BuildNotImplementedException(IndentedStringBuilder b, ISymbol member, bool forceRaise)
		{
			...
			if (forceRaise)
			{
				b.AppendLineInvariant(
					$"throw new global::System.NotImplementedException(\"The member {memberName} is not implemented. For more information, visit https://github.com/ellisnet/CodeBrix.Platform/blob/main/NOT-IMPLEMENTED.md\");"
				);
			}
			else
			{
			...
			foreach (var method in type.GetMembers().OfType<IMethodSymbol>())
			{
				...
				)
				{
					if (methods.HasUndefined)
					{
						...
						b.AppendLineInvariant($"[global::CodeBrix.Platform.NotImplemented({methods.GenerateNotImplementedList()})]");
		...
		protected void BuildProperties(INamedTypeSymbol type, IndentedStringBuilder b, PlatformSymbols<INamedTypeSymbol> types)
		{
			foreach (var property in type.GetMembers().OfType<IPropertySymbol>())
			{
				...
				if (allMembers.HasUndefined)
				{
					...
					{
						b.AppendLineInvariant($"[global::CodeBrix.Platform.NotImplemented({allMembers.GenerateNotImplementedList()})]");
		...
		private static async Task<Compilation> LoadProject(string projectFile, string targetFramework)
		{
			...
			foreach (var expectedRef in expectedRefs)
			{
				if (!externalCompilationReferences.Contains(expectedRef))
				{
					...
					// Another reason for hitting this is if you define CodeBrixTargetFrameworkOverride, say to net7.0.
		...
		private static async Task<Compilation> InnerLoadProject(string projectFile, string targetFramework)
		{
			...
			// can no longer hook a BinaryLogger directly into the workspace's MSBuild
			// invocation. Capture every WorkspaceFailed diagnostic to a per-project text
			// log instead, mirroring what the binlog provided for triage purposes.
			var diagnosticsDir = Path.Combine(Directory.GetCurrentDirectory(), "binlogs");
			Directory.CreateDirectory(diagnosticsDir);
			var diagnosticsLogPath = Path.Combine(diagnosticsDir, $"{projectFileName}_{targetFramework}.diagnostics.log");
			File.Delete(diagnosticsLogPath);

			ws.RegisterWorkspaceFailedHandler(e =>
			{
				var line = $"[{e.Diagnostic.Kind}] {e.Diagnostic}";
			...
			if (metadataLessProjects.Length > 0)
			{
				...
				throw new InvalidOperationException(
					...
					"This may also be related to a missing default configuration directive. Refer to the CodeBrix.Platform.SourceGenerator Readme.md file for more details."
				);
			}
