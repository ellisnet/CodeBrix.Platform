...
internal sealed class FrameBufferNativeOpenGLWrapper : INativeOpenGLWrapper
{
	...
	// Inlined here on the emulated head: the DRM renderer (and its LibDrm
	// bindings) do not exist in this project, and this is the only libgbm
	// entry point the wrapper needs beyond the destroy above.
	[DllImport("libgbm.so.1")]
