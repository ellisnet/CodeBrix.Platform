...
{
	private const string WindowClassName = "CodeBrixPlatformRegularWindow";
	...
	{
		this.LogInfo()?.Info($"Forcibly closing window {((nint)_hwnd).ToString("X", CultureInfo.InvariantCulture)}");
	...
	public override unsafe void SetIcon(string iconPath)
	{
		...
		PInvoke.SendMessage(_hwnd, PInvoke.WM_SETICON, PInvoke.ICON_SMALL, (nint)hIcon.Value);
		PInvoke.SendMessage(_hwnd, PInvoke.WM_SETICON, PInvoke.ICON_BIG, (nint)hIcon.Value);
	}
