...
namespace CodeBrix.Platform.UI.Runtime.Skia.Win32 //Was previously: Uno.UI.Runtime.Skia.Win32
{
	...
	{
		public static readonly uint CodeBrixWin32DispatcherMsg;
		...
		{
			CodeBrixWin32DispatcherMsg = PInvoke.RegisterWindowMessage(nameof(CodeBrixWin32DispatcherMsg));
			if (CodeBrixWin32DispatcherMsg == 0)
			{
				throw new InvalidOperationException($"Failed to register Dispatcher Win32 message: {Win32Helper.GetErrorMessage()}");
			}

			using var lpClassName = new Win32Helper.NativeNulTerminatedUtf16String("CodeBrixPlatformDispatcherWindow");
		...
		{
			if (msg == CodeBrixWin32DispatcherMsg)
			{
				Action action;
				lock (_actions)
				{
		...
		public static void Schedule(Action action, NativeDispatcherPriority p)
		{
			...
			_ = PInvoke.PostMessage(_hwnd, CodeBrixWin32DispatcherMsg, new WPARAM(), new LPARAM());
		}
