...
//was previously: ICSharpCode.AvalonEdit/TextEditorOptions.cs in the AvalonEdit repo (MIT), where
//the class was named TextEditorOptions; renamed per the port naming rules. [Serializable] is
//dropped (this library does not support binary serialization), but [field: NonSerialized] stays
//on the PropertyChanged event because the copy constructor uses that attribute to know which
//fields not to copy. EnableImeSupport and EnableTextDragDrop remain as properties for
//compatibility, but the features they control are not available in this version of the editor.

/// <summary>
/// A container for the text editor options.
/// </summary>
public class AdvancedTextEditOptions : INotifyPropertyChanged
{
	#region ctor
	/// <summary>
	/// Initializes an empty instance of AdvancedTextEditOptions.
	/// </summary>
	public AdvancedTextEditOptions()
	{
	}

	/// <summary>
	/// Initializes a new instance of AdvancedTextEditOptions by copying all values
	/// from <paramref name="options"/> to the new instance.
	/// </summary>
	public AdvancedTextEditOptions(AdvancedTextEditOptions options)
	{
		// get all the fields in the class
		FieldInfo[] fields = typeof(AdvancedTextEditOptions).GetFields(BindingFlags.NonPublic | BindingFlags.Public | BindingFlags.Instance);
	...
	public virtual bool EnableEmailHyperlinks
	{
		...
		set
		{
			if (enableEmailHyperlinks != value)
			{
				...
				//was previously: the upstream file raises this change with the historical name
				//"EnableEMailHyperlinks" (capital M), not nameof(EnableEmailHyperlinks); kept
				//verbatim so listeners ported from upstream keep working.
	...
	public virtual int IndentationSize
	{
		...
		set
		{
			...
			// sanity check; a too large value might crash the text layout layer internally much later
			// (it only crashed in the hundred thousands upstream; but might crash earlier with larger fonts)
			if (value > 1000)
			{
	...
	/// <summary>
	/// Gets/Sets whether text can be dragged within the text area.
	/// Text drag-and-drop is not available in this version of the editor; this option is
	/// reserved for a future version and currently has no effect.
	/// </summary>
	[DefaultValue(true)]
	...
	/// <summary>
	/// Gets/Sets whether support for Input Method Editors (IME)
	/// for non-alphanumeric scripts (Chinese, Japanese, Korean, ...) is enabled.
	/// IME support is not available in this version of the editor; this option is
	/// reserved for a future version and currently has no effect.
	/// </summary>
	[DefaultValue(true)]
