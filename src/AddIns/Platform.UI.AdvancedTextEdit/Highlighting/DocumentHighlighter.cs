...
//was previously: ICSharpCode.AvalonEdit/Highlighting/DocumentHighlighter.cs in the AvalonEdit repo (MIT).
...
public class DocumentHighlighter : ILineTracker, IHighlighter
{
	...
	public SpanStack GetSpanStack(int lineNumber)
	{
		...
		// The list contains null only for lines that have not been scanned yet;
		// UpdateHighlightingState guarantees the requested line has been scanned.
	...
	{
		return GetSpanStack(lineNumber).Select(s => s.SpanColor).Where(s => s != null).Select(s => s!);
	}
