...
//was previously: ICSharpCode.AvalonEdit/Highlighting/HighlightingColorizer.cs in the AvalonEdit
//repo (MIT). The highlighting state machinery is transliterated; only ApplyColorToElement changed
//with the text stack: instead of composing a WPF Typeface, it writes the discrete
//family/weight/style/size values and the underline/strikethrough decorations onto the element's
//VisualLineElementTextRunProperties, which the visual line bakes into its engine layout.

/// <summary>
/// A colorizer that interprets a highlighting rule set and colors the document accordingly.
/// </summary>
public class HighlightingColorizer : DocumentColorizingTransformer
{
	...
	{
		if (sender is TextView senderTextView)
		{
			DeregisterServices(senderTextView);
			RegisterServices(senderTextView);
		}
	}
	...
	protected override void ColorizeLine(DocumentLine line)
	{
		if (highlighter != null)
		{
			...
			foreach (HighlightedSection section in hl.Sections)
			{
				...
				if (color == null || IsEmptyColor(color))
					continue;
				ChangeLinePart(section.Offset, section.Offset + section.Length,
							   visualLineElement => ApplyColorToElement(visualLineElement, color));
			}
		}
	...
	internal static void ApplyColorToElement(VisualLineElement element, HighlightingColor color, ITextRunConstructionContext? context)
	{
		...
		if (color.Foreground != null)
		{
			...
			if (b != null)
				properties.SetForegroundBrush(b);
		}
		...
		//was previously: upstream rebuilt a WPF Typeface from family/style/weight; the port's run
		//properties carry those values discretely.
		if (color.FontStyle != null)
			properties.SetFontStyle(color.FontStyle.Value);
		if (color.FontWeight != null)
			properties.SetFontWeight(color.FontWeight.Value);
		if (color.FontFamily != null)
			properties.SetFontFamily(color.FontFamily.Source);
		if (color.Underline ?? false)
			properties.SetTextDecorations(TextDecorations.Underline);
		if (color.Strikethrough ?? false)
			properties.SetTextDecorations(TextDecorations.Strikethrough);
		if (color.FontSize.HasValue)
			properties.SetFontSize(color.FontSize.Value);
	}
	...
	void OnHighlightStateChanged(int fromLineNumber, int toLineNumber)
	{
		...
		var currentTextView = this.textView;
		...
		{
			currentTextView.Redraw(currentTextView.Document.GetLineByNumber(fromLineNumber));
		}
		else
		{
			...
			var fromLine = currentTextView.Document.GetLineByNumber(fromLineNumber);
			var toLine = currentTextView.Document.GetLineByNumber(toLineNumber);
			int startOffset = fromLine.Offset;
			currentTextView.Redraw(startOffset, toLine.EndOffset - startOffset);
		}
