...
//was previously: ICSharpCode.AvalonEdit/Rendering/HeightTree.cs in the AvalonEdit repo (MIT).
//This red-black tree is the scrolling backbone; its algorithms are transliterated unchanged.
//The only additions are nullable annotations: fields and locals that can genuinely be null are
//declared as nullable, and the data-structure invariants the type system cannot see (a red node
//always has a parent, a collapsed section in the tree has non-null Start/End, ...) are recorded
//as Debug.Assert calls, which also drive the compiler's null-state analysis. If such an
//invariant were violated, release builds fail with a NullReferenceException exactly as upstream.
		...
		if (node.lineNode.collapsedSections != null)
		{
			foreach (CollapsedLineSection cs in node.lineNode.collapsedSections.ToArray())
			{
				...
				else if (cs.Start == line)
				{
					...
					Debug.Assert(cs.Start != null && cs.End != null);
...
//		}
	...
	void BeforeNodeReplace(HeightTreeNode removedNode, HeightTreeNode newNode, HeightTreeNode newNodeOldParent)
	{
		...
		while (newNodeOldParent != removedNode)
		{
			...
			Debug.Assert(newNodeOldParent.parent != null);
	...
	{
		Debug.Assert(inRemoval);
		Debug.Assert(nodesToCheckForMerging != null);
		inRemoval = false;
	...
	internal void CheckProperties()
	{
		...
		{
			Debug.Assert(cs.Start != null && cs.End != null);
			var startSections = GetNode(cs.Start).lineNode.collapsedSections;
			Debug.Assert(startSections != null && startSections.Contains(cs));
			var endSections = GetNode(cs.End).lineNode.collapsedSections;
			Debug.Assert(endSections != null && endSections.Contains(cs));
	...
	void CheckProperties(HeightTreeNode node)
	{
		...
		if (node.lineNode.IsDirectlyCollapsed)
			Debug.Assert(node.lineNode.collapsedSections != null && node.lineNode.collapsedSections.Count > 0);
		if (node.left != null)
		{
		...
		if (node.IsDirectlyCollapsed)
		{
			Debug.Assert(node.collapsedSections != null && node.collapsedSections.Count > 0);
			totalHeight = 0;
		}
	...
	void FixTreeOnInsert(HeightTreeNode node)
	{
		...
		Debug.Assert(parentNode.parent != null);
	...
	void RemoveNode(HeightTreeNode removedNode)
	{
		if (removedNode.left != null && removedNode.right != null)
		{
			...
			Debug.Assert(leftMost.parent != null);
	...
	void AddRemoveCollapsedSection(CollapsedLineSection section, int sectionLength, bool add)
	{
		...
		Debug.Assert(section.Start != null && section.End != null);
