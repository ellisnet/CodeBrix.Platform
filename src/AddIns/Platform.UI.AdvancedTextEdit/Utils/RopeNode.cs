...
//was previously: ICSharpCode.AvalonEdit/Utils/RopeNode.cs in the AvalonEdit repo (MIT).
//The algorithm is transliterated unchanged. Nullable re-expression only: left/right/contents are
//annotated as nullable (they really are null depending on the node kind), and Debug.Assert calls
//encode the node-kind invariants (concat nodes have children, leaf nodes have contents) where the
//upstream code relied on them implicitly.
...
class RopeNode<T>
{
	...
	internal int Balance
	{
		get
		{
			Debug.Assert(left != null && right != null); // only concat nodes have a balance
	...
	internal void Rebalance()
	{
		...
		if (left == null)
			return;
		Debug.Assert(right != null); // a node with a left child is a concat node
		...
		while (Math.Abs(this.Balance) > 1)
		{
			...
			Debug.Assert(left != null && right != null); // rotations keep this node a concat node
		}
	...
	{
		Debug.Assert(!isShared);
		Debug.Assert(left != null && right != null && right.left != null && right.right != null);
	...
	{
		Debug.Assert(!isShared);
		Debug.Assert(left != null && right != null && left.left != null && left.right != null);
	...
	void MergeIfPossible()
	{
		...
		{
			Debug.Assert(left != null && right != null); // only concat nodes are merged
	...
	internal RopeNode<T> StoreElements(int index, T[] array, int arrayIndex, int count)
	{
		...
		if (result.height == 0)
		{
			Debug.Assert(result.contents != null); // leaf node
			// leaf node:
			Array.Copy(array, arrayIndex, result.contents, index, count);
		}
		else
		{
			Debug.Assert(result.left != null && result.right != null && this.left != null); // concat node
			// concat node:
	...
	internal void CopyTo(int index, T[] array, int arrayIndex, int count)
	{
		...
		{
			Debug.Assert(left != null && right != null); // concat node
			// concat node
	...
	internal RopeNode<T> SetElement(int offset, T value)
	{
		...
		{
			Debug.Assert(result.left != null && result.right != null); // concat node
	...
	internal RopeNode<T> Insert(int offset, T[] array, int arrayIndex, int count)
	{
		...
		if (this.length + count < RopeNode<char>.NodeSize)
		{
			...
			Debug.Assert(result.contents != null);
