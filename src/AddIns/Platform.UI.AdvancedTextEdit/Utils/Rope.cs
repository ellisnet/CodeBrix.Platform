...
//was previously: ICSharpCode.AvalonEdit/Utils/Rope.cs in the AvalonEdit repo (MIT).
//The algorithm is transliterated unchanged. Nullable re-expression only: the last-used-node cache
//is annotated nullable, and Debug.Assert calls encode the leaf-node invariant where the upstream
//code relied on it implicitly. The string-input constructor assigns the root through a node cast
//instead of upstream's ((Rope<char>)(object)this).root cast, so the compiler can see the
//field assignment; the runtime behavior (InvalidCastException when T is not char) is identical.
...
public sealed class Rope<T> : IList<T>, ICloneable
{
	...
	public Rope(IEnumerable<T> input)
	{
		...
		else
		{
			...
			if (text != null)
			{
				...
				this.root = (RopeNode<T>)(object)CharRope.InitFromString(text);
			}
			else
			{
				T[] arr = ToArray(input);
	...
	public void InsertRange(int index, Rope<T> newElements)
	{
		...
		{
			throw new ArgumentOutOfRangeException(nameof(index), index, "0 <= index <= " + this.Length.ToString(CultureInfo.InvariantCulture));
		}
		if (newElements == null)
	...
	public T this[int index]
	{
		get
		{
			...
			Debug.Assert(entry.node.contents != null); // FindNodeUsingCache always returns a leaf node
	...
	internal ImmutableStack<RopeCacheEntry> FindNodeUsingCache(int index)
	{
		...
		while (true)
		{
			...
			Debug.Assert(entry.node.left != null && entry.node.right != null); // concat node
	...
	internal void VerifyRange(int startIndex, int length)
	{
		...
		{
			throw new ArgumentOutOfRangeException(nameof(startIndex), startIndex, "0 <= startIndex <= " + this.Length.ToString(CultureInfo.InvariantCulture));
		}
		if (length < 0 || startIndex + length > this.Length)
		{
			throw new ArgumentOutOfRangeException(nameof(length), length, "0 <= length, startIndex(" + startIndex + ")+length <= " + this.Length.ToString(CultureInfo.InvariantCulture));
		}
	}
	...
	public int IndexOf(T item, int startIndex, int count)
	{
		...
		while (count > 0)
		{
			...
			Debug.Assert(contents != null); // FindNodeUsingCache always returns a leaf node
	...
	static IEnumerator<T> Enumerate(RopeNode<T>? node)
	{
		...
		while (node != null)
		{
			...
			while (node.contents == null)
			{
				...
				Debug.Assert(node.left != null && node.right != null); // concat node
