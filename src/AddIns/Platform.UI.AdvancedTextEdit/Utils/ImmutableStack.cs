...
//was previously: ICSharpCode.AvalonEdit/Utils/ImmutableStack.cs in the AvalonEdit repo (MIT).
//Nullable re-expression only: the empty sentinel stores default(T) explicitly, empty-stack checks
//read the next field directly (IsEmpty is defined as next == null), and PeekOrDefault is annotated
//as returning a possibly-default value. Behavior is unchanged.
...
public sealed class ImmutableStack<T> : IEnumerable<T>
{
	...
	{
		// Only the Empty sentinel carries default(T) here; Peek() guards it behind IsEmpty and
		// PeekOrDefault() is documented to return default(T) for the empty stack.
		this.value = default!;
	}
	...
	{
		ImmutableStack<T>? next = this.next;
		if (next == null)
			throw new InvalidOperationException("Operation not valid on empty stack.");
		return next;
	}
