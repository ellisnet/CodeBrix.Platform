...
//was previously: ICSharpCode.AvalonEdit/Utils/CharRope.cs in the AvalonEdit repo (MIT).
//Transliterated unchanged; Debug.Assert calls encode the leaf/concat node invariants where the
...
public static class CharRope
{
	...
	static void FillNode(RopeNode<char> node, string text, int start)
	{
		...
		{
			Debug.Assert(node.left != null && node.right != null); // CreateNodes only creates leaf and concat nodes
