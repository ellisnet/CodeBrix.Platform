...
//was previously: ICSharpCode.AvalonEdit/Utils/CompressingTreeList.cs in the AvalonEdit repo (MIT).
//The red-black tree algorithm is transliterated unchanged. Nullable re-expression only: node
//links and the root are annotated as nullable, and Debug.Assert calls encode the red-black tree
//invariants (existing sibling/grandparent nodes) where the upstream code relied on them
//implicitly.
...
public sealed class CompressingTreeList<T> : IList<T>
{
	...
	public void InsertRange(int index, int count, T item)
	{
		if (index < 0 || index > this.Count)
			throw new ArgumentOutOfRangeException(nameof(index), index, "Value must be between 0 and " + this.Count);
		if (count < 0)
	...
	public void RemoveRange(int index, int count)
	{
		...
		if (count < 0 || index + count > this.Count)
			throw new ArgumentOutOfRangeException(nameof(count), count, "0 <= length, index(" + index + ")+count <= " + this.Count);
		if (count == 0)
			return;
	...
	{
		Debug.Assert(root != null); // callers only search in non-empty trees
		Node node = root;
		while (true)
		{
	...
	public T this[int index]
	{
		...
		{
			if (index < 0 || index >= this.Count)
				throw new ArgumentOutOfRangeException(nameof(index), index, "Value must be between 0 and " + (this.Count - 1));
	...
	public void CopyTo(T[] array, int arrayIndex)
	{
		...
		if (arrayIndex < 0 || arrayIndex + this.Count > array.Length)
			throw new ArgumentOutOfRangeException(nameof(arrayIndex), arrayIndex, "Value must be between 0 and " + (array.Length - this.Count));
		foreach (T v in this)
		{
			array[arrayIndex++] = v;
		}
	...
	void FixTreeOnInsert(Node node)
	{
		...
		Debug.Assert(grandparentNode != null);
		...
		if (node == parentNode.right && parentNode == grandparentNode.left)
		{
			...
			Debug.Assert(node.left != null); // the rotation made the old parent the left child
			node = node.left;
		}
		else if (node == parentNode.left && parentNode == grandparentNode.right)
		{
			RotateRight(parentNode);
			Debug.Assert(node.right != null); // the rotation made the old parent the right child
			node = node.right;
		}
	...
	void FixTreeOnDelete(Node? node, Node? parentNode)
	{
		...
		Debug.Assert(sibling != null); // red-black invariant: the sibling of a double-black node exists
		...
		if (node == parentNode.left &&
			...
			GetColor(sibling.right) == BLACK)
		{
			Debug.Assert(sibling.left != null); // GetColor returned RED, so the node exists
			sibling.color = RED;
			...
			RotateRight(sibling);
				 ...
				 GetColor(sibling.left) == BLACK)
		{
			Debug.Assert(sibling.right != null); // GetColor returned RED, so the node exists
			sibling.color = RED;
	...
	{
		Debug.Assert(node.parent != null); // callers only ask for siblings of non-root nodes
