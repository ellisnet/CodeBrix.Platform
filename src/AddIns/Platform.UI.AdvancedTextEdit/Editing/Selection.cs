...
//was previously: ICSharpCode.AvalonEdit/Editing/Selection.cs in the AvalonEdit repo (MIT).
//The selection model is transliterated. CreateDataObject builds a
//Windows.ApplicationModel.DataTransfer.DataPackage instead of a WPF DataObject; the WPF-only
//typeof(string).FullName format (a drag'n'drop nicety for SciTe) is dropped together with
//drag'n'drop support. SurroundingSegment is honestly annotated nullable (it is null for the
//empty selection, as upstream documented).
...
public abstract class Selection
{
	...
	public virtual bool Contains(int offset)
	{
		if (this.IsEmpty)
			return false;
		if (this.SurroundingSegment is ISegment surroundingSegment && surroundingSegment.Contains(offset, 0))
		{
	...
	/// <summary>
	/// Creates a data package containing the selection's text.
	/// </summary>
	public virtual DataPackage CreateDataObject(TextArea textArea)
	{
		...
		if (EditingCommandHandler.ConfirmDataFormat(textArea, data, StandardDataFormats.Text))
		{
			data.SetText(text);
		}

		//was previously: the text was also stored under typeof(string).FullName to enable
		//drag'n'drop into SciTe; drag'n'drop is out of scope and the format is WPF-specific.

		// Also copy text in HTML format to clipboard - good for pasting text into Word
		// or into other rich text editors.
		if (EditingCommandHandler.ConfirmDataFormat(textArea, data, StandardDataFormats.Html))
		{
