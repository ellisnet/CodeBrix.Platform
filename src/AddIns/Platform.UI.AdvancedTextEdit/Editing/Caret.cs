...
//was previously: ICSharpCode.AvalonEdit/Editing/Caret.cs in the AvalonEdit repo (MIT).
//The position/validation logic is transliterated. Divergences:
//- The Win32 accessibility caret (Win32.CreateCaret/SetCaretPosition, used by the Windows
//  Magnifier's 'Follow text editing') has no counterpart and is dropped, as is the IME
//  composition-window update.
//- The caret layer is an IBackgroundRenderer registered on the text view's paint pass instead of
//  a UIElement adorner layer; scroll-offset changes invalidate that layer.
//- Show() defers through DispatcherQueue.TryEnqueue instead of Dispatcher.BeginInvoke; without a
//  dispatcher (host-free unit tests) it runs inline.
//- The caret width is a 1 DIP constant (upstream read SystemParameters.CaretWidth), and the
//  overstrike rectangle derives from caret x-positions instead of WPF TextLine.GetTextBounds.
...
{
	/// <summary>
	/// The width of the caret rectangle, in device-independent pixels.
	/// </summary>
	//was previously: SystemParameters.CaretWidth; constant per the port's drawing rules.
	const double CaretWidth = 1.0;

	readonly TextArea textArea;
	readonly TextView textView;
	readonly CaretLayer caretLayer;
	bool visible;
	...
	internal Caret(TextArea textArea)
	{
		...
		caretLayer = new CaretLayer(textArea);
		//was previously: textView.InsertLayer(caretLayer, KnownLayer.Caret, LayerInsertionPosition.Replace);
		//layers are background-renderer draw phases in this port.
		textView.BackgroundRenderers.Add(caretLayer);
	...
	internal void OnDocumentChanged(DocumentChangeEventArgs e)
	{
		...
		if (storedCaretOffset >= 0)
		{
			...
			if (textArea.Selection.SurroundingSegment is ISegment surroundingSegment && storedCaretOffset == surroundingSegment.EndOffset)
	...
	Rect CalcCaretRectangle(VisualLine visualLine)
	{
		...
		TextLineLayout textLine = visualLine.GetTextLine(position.VisualColumn, position.IsAtEndOfLine);
	...
	Rect CalcCaretOverstrikeRectangle(VisualLine visualLine)
	{
		...
		TextLineLayout textLine = visualLine.GetTextLine(currentPos);

		//was previously: inside the text, upstream used WPF TextLine.GetTextBounds for correct
		//rectangles in bidirectional text; this port derives the block from the two caret
		//x-positions in every case, so the block may span differently inside RTL runs.
		double xPos = visualLine.GetTextLineVisualXPosition(textLine, currentPos);
		double xPos2 = nextPos >= 0 ? visualLine.GetTextLineVisualXPosition(textLine, nextPos) : xPos;
		double lineTop = visualLine.GetTextLineVisualYPosition(textLine, VisualYPosition.TextTop);
		double lineBottom = visualLine.GetTextLineVisualYPosition(textLine, VisualYPosition.TextBottom);
		Rect r = new Rect(Math.Min(xPos, xPos2), lineTop, Math.Abs(xPos2 - xPos), lineBottom - lineTop);
	...
	{
		//was previously: the constant lived on Caret; it moved to TextView so that the scrolling
		//surface and the caret stay in sync (see TextView.MinimumDistanceToViewBorder).
		BringCaretToView(TextView.MinimumDistanceToViewBorder);
	}
	...
	internal void BringCaretToView(double border)
	{
		...
		{
			//was previously: caretRectangle.Inflate(border, border); the Rect.Inflate method is
			//not public in this framework.
			caretRectangle = new Rect(caretRectangle.X - border,
									  caretRectangle.Y - border,
									  caretRectangle.Width + 2 * border,
									  caretRectangle.Height + 2 * border);
	...
	public void Show()
	{
		...
		if (!showScheduled)
		{
			...
			{
				dispatcherQueue.TryEnqueue(ShowInternal);
			}
			else
			{
				// host-free (unit tests): no dispatcher to defer to
				ShowInternal();
			}
		}
	}
	...
	void ShowInternal()
	{
		...
		if (textView != null)
		{
			VisualLine? visualLine = textView.Document != null ? textView.GetVisualLine(position.Line) : null;
			if (visualLine != null)
			{
				Rect caretRect = this.textArea.OverstrikeMode ? CalcCaretOverstrikeRectangle(visualLine) : CalcCaretRectangle(visualLine);
				//was previously: a Win32 caret was created/positioned here so that assistive
				//technology (e.g. the Windows Magnifier) could follow the caret; dropped.
				caretLayer.Show(caretRect);
			}
			else
			{
				caretLayer.Hide();
	...
	public Brush? CaretBrush {
		get { return caretLayer.CaretBrush; }
		set {
			caretLayer.CaretBrush = value;
			textView.InvalidateLayer(KnownLayer.Caret);
		}
	}
}
