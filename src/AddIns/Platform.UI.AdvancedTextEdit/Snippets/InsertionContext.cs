...
//was previously: ICSharpCode.AvalonEdit/Snippets/InsertionContext.cs in the AvalonEdit repo (MIT).
//Transliterated; the weak document listener runs through this port's WeakEventManagerBase shim
//(same public shape as the WPF weak event manager), and the constructor throws when the text area
//has no document (upstream failed later with a NullReferenceException).
...
public class InsertionContext : IWeakEventListener
{
	...
	public InsertionContext(TextArea textArea, int insertionPosition)
	{
		...
		this.Document = textArea.Document ?? throw ThrowUtil.NoDocumentAssigned();
	...
	public IActiveElement? GetActiveElement(SnippetElement owner)
	{
		if (owner == null)
			throw new ArgumentNullException(nameof(owner));
		if (elementMap.TryGetValue(owner, out IActiveElement? element))
			return element;
		else
			return null;
	}
	...
	protected virtual bool ReceiveWeakEvent(Type managerType, object? sender, EventArgs e)
	{
		if (managerType == typeof(TextDocumentWeakEventManager.UpdateFinished))
		{
			...
			if (wholeSnippetAnchor != null && wholeSnippetAnchor.Length == 0 && deactivateIfSnippetEmpty)
	...
	public void Link(ISegment mainElement, ISegment[] boundElements)
	{
		...
		if (boundElements == null)
			throw new ArgumentNullException(nameof(boundElements));
