...
//was previously: ICSharpCode.AvalonEdit/Document/LineManager.cs in the AvalonEdit repo (MIT).
//UpdateListOfLineTrackers is annotated [MemberNotNull] so the constructor's call to it satisfies
//nullable analysis for the lineTrackers field, and Debug.Assert null checks were added where the
//line tree structure guarantees a neighboring line exists. No behavior changed.
...
sealed class LineManager
{
	...
	[MemberNotNull(nameof(lineTrackers))]
		...
		else
		{
			...
			else if (lastChar == '\n')
			{
				...
				else if (newTotalLength == 1 && lineOffset > 0 && document.GetCharAt(lineOffset - 1) == '\r')
				{
					...
					Debug.Assert(previousLine != null);
					RemoveLine(line);
