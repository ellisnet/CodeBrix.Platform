...
//was previously: ICSharpCode.AvalonEdit/Document/TextSegmentCollection.cs in the AvalonEdit repo (MIT);
...
public sealed class TextSegmentCollection<T> : ICollection<T>, ISegmentTree, IWeakEventListener where T : TextSegment
{
	...
	void AddSegment(TextSegment node)
	{
		...
		if (root == null) {
			...
			Debug.Assert(n != null); // insertionOffset < root.totalNodeLength, so FindNode always finds a node
