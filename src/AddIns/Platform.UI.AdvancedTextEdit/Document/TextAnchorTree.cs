...
//was previously: ICSharpCode.AvalonEdit/Document/TextAnchorTree.cs in the AvalonEdit repo (MIT);
...
sealed class TextAnchorTree
{
	...
	void InsertText(int offset, int length, bool defaultAnchorMovementIsBeforeInsertion)
	{
		...
		if (offset == root.totalLength) {
			...
			Debug.Assert(endNode != null); // offset < root.totalLength, so FindNode always finds a node
	...
	TextAnchorNode FindActualBeginNode(TextAnchorNode? node)
	{
		...
		if (node == null) {
			...
			Debug.Assert(root != null); // the callers only run on a non-empty tree
			node = root.LeftMost;
		}
		return node;
	}
	...
	void PerformInsertText(TextAnchorNode beginNode, TextAnchorNode? endNode, int length, bool defaultAnchorMovementIsBeforeInsertion)
	{
		...
		while (temp != endNode) {
			Debug.Assert(temp != null); // the walk stays inside [beginNode, endNode); only a null endNode ends it at null
		...
		foreach (TextAnchorNode node in beforeInsert) {
			Debug.Assert(temp != null); // beforeInsert holds at most the nodes of [beginNode, endNode)
	...
	void SwapAnchors(TextAnchorNode n1, TextAnchorNode n2)
	{
		if (n1 != n2) {
			...
			if (anchor1 == null) {
				...
				Debug.Assert(anchor2 != null); // anchor1 and anchor2 cannot both be null (checked above)
				anchor2.node = n1;
	...
	public TextAnchor CreateAnchor(int offset)
	{
		...
		if (root == null) {
			...
			Debug.Assert(n != null); // offset < root.totalLength, so FindNode always finds a node
