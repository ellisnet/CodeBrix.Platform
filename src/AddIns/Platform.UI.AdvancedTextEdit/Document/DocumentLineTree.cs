...
//was previously: ICSharpCode.AvalonEdit/Document/DocumentLineTree.cs in the AvalonEdit repo (MIT);
//converted mechanically (license header removed, namespace re-rooted, file-scoped), then
//reviewed for this framework. Algorithms are transliterated, not restructured.
...
sealed class DocumentLineTree : IList<DocumentLine>
{
	...
	public void RebuildTree(List<DocumentLine> documentLines)
	{
		...
		LineNode? newRoot = BuildTree(nodes, 0, nodes.Length, height);
		Debug.Assert(newRoot != null); // nodes.Length > 0 (asserted above), so BuildTree returns a node
		root = newRoot;
		root.color = BLACK;
#if DEBUG
		CheckProperties();
		...
		while (true) {
			if (node.left != null && index < node.left.nodeTotalCount) {
				...
				if (index == 0)
					return node;
				index--;
				Debug.Assert(node.right != null); // index is within this subtree, so a right child must exist
				node = node.right;
			}
		}
	}
	...
	LineNode GetNodeByOffset(int offset)
	{
		...
		while (true) {
			if (node.left != null && offset < node.left.nodeTotalLength) {
				...
				if (offset < 0)
					return node;
				Debug.Assert(node.right != null); // offset is within this subtree, so a right child must exist
				node = node.right;
			}
		}
	}
	...
	void FixTreeOnInsert(LineNode node)
	{
		...
		Debug.Assert(grandparentNode != null); // see comment above
		...
		if (node == parentNode.right && parentNode == grandparentNode.left) {
			...
			Debug.Assert(node.left != null); // the rotation made the old parent the left child of node
			node = node.left;
		} else if (node == parentNode.left && parentNode == grandparentNode.right) {
			RotateRight(parentNode);
			Debug.Assert(node.right != null); // the rotation made the old parent the right child of node
			node = node.right;
		}
		// because node might have changed, reassign variables:
		parentNode = node.parent;
		Debug.Assert(parentNode != null); // node is below the grandparent, so it always has a parent
	...
	void FixTreeOnDelete(LineNode? node, LineNode? parentNode)
	{
		...
		if (node == parentNode.left &&
			...
			GetColor(sibling.right) == BLACK) {
			sibling.color = RED;
			Debug.Assert(sibling.left != null); // GetColor(sibling.left) == RED means the child exists
			...
			RotateRight(sibling);
				   ...
				   GetColor(sibling.left) == BLACK) {
			sibling.color = RED;
			Debug.Assert(sibling.right != null); // GetColor(sibling.right) == RED means the child exists
	...
	void ReplaceNode(LineNode replacedNode, LineNode? newNode)
	{
		if (replacedNode.parent == null) {
			...
			//was previously: root = newNode; annotated for the tree invariant that a document
			//always contains at least one line, so the root is never replaced with null.
			root = newNode ?? throw new InvalidOperationException("The document line tree must always contain at least one line.");
		} else {
	...
	{
		Debug.Assert(node.parent != null); // callers pass a red node, which is never the root
	...
	static LineNode Sibling(LineNode? node, LineNode parentNode)
	{
		...
		Debug.Assert(sibling != null); // in the delete fixup, the sibling subtree contains black nodes and cannot be empty
		return sibling;
	}
