...
//was previously: ICSharpCode.AvalonEdit/Document/TextSourceVersionProvider.cs in the AvalonEdit repo (MIT).
//Upstream's Utils.Empty<T>.Array (a pre-BCL helper) is replaced by Array.Empty<T>().
...
public class TextSourceVersionProvider
{
	...
	sealed class Version : ITextSourceVersion
	{
		...
		IEnumerable<TextChangeEventArgs> GetForwardChanges(Version other)
		{
			...
			{
				// Every version between this one and the current version carries its change
				// to the next version; the loop never walks past the current version.
				Debug.Assert(node != null && node.change != null);
