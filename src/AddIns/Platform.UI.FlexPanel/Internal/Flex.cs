...
//
// Ported to CodeBrix.Platform for the FlexPanel add-in from dotnet/maui
// src/Core/src/Layouts/Flex.cs (MIT). See THIRD-PARTY-NOTICES.txt at the repo root.
...
//was previously: namespace Microsoft.Maui.Layouts.Flex
namespace CodeBrix.Platform.UI.FlexPanel.Internal;
...
class Item : List<Item>
{
		...
		for (int i = 0; i < item.Count; i++)
		{
			...
			if (!child.Basis.IsAuto)
			{
				if (child.Basis.Length < 0)
					throw new InvalidOperationException("basis should >=0"); //was previously: throw new Exception(...)
				if (child.Basis.IsRelative && child.Basis.Length > 1)
					throw new InvalidOperationException("relative basis should be <=1"); //was previously: throw new Exception(...)
			...
			//
			// for FlexPanel: upstream MAUI compiles this only #if ANDROID || WINDOWS; it is applied
			// unconditionally here because every CodeBrix.Platform head can run at a fractional DPI
			// scale factor, and upstream documents the tolerance as having no adverse effects.
			...
			if (child.Grow < 0
				|| child.Shrink < 0)
				throw new InvalidOperationException("shrink and grow should be >= 0"); //was previously: throw new Exception(...)

			layout.flex_grows += child.Grow;
			layout.flex_shrinks += child.Shrink;

			// for FlexPanel: upstream dotnet/maui wraps this subtraction in `if (flex_dim > 0)`
			// (commented there as protecting the unconstrained-measure case, where flex_dim starts
			// at zero). As written, that guard also stops the tally as soon as children consume
			// exactly all the space, so overflow contributed by later children is never counted
			// and shrink silently stops working. The original C engine subtracts unconditionally,
			// letting flex_dim go negative so the shrink pass can distribute the overflow. The
			// unconstrained-measure case the upstream guard protected is handled at the panel
			// level by building items with Shrink=0 (the same measure hack MAUI's panel uses).
	...
	static void layout_items(Item item, int child_begin, int child_end, int children_count, ref flex_layout layout, bool inMeasureMode)
	{
		...
		for (int i = child_begin; i < child_end; i++)
		{
			...
			switch (child_align(child, item))
			{
				...
				default:
					throw new InvalidOperationException(); //was previously: throw new Exception()
			}
	...
	struct flex_layout
	{
		...
		public void init(Item item, float width, float height)
		{
			...
			// for FlexPanel: the dotnet/maui port of these two lines reads
			//   width - item.PaddingLeft + item.PaddingRight   (and the same for height),
			// a sign slip against the original C engine (flex.c subtracts BOTH paddings).
			// MAUI never exercises the slip because its panel subtracts padding before invoking
			// the engine and leaves engine-level padding at zero - exactly as FlexPanel does -
			// so restoring the C behavior changes nothing at the panel level while keeping the
			// engine correct for direct (test-suite) consumers.
