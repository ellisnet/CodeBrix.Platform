...
/// </summary>
[Collection(GraphicsSerializedCollection.Name)]
public class VideoPresenterTests
{
    [Fact]
    public void fill_stretches_the_picture_over_the_whole_destination()
    {
        ...
        //Act
        SKRect target = VideoPresenter.ComputeDestinationRect(destination, 64, 36, VideoStretch.Fill);
    ...
    public void uniform_letterboxes_a_wide_picture_in_a_square_destination()
    {
        ...
        //Act
        SKRect target = VideoPresenter.ComputeDestinationRect(destination, 200, 100, VideoStretch.Uniform);
    ...
    public void uniform_pillarboxes_a_tall_picture_in_a_wide_destination()
    {
        ...
        //Act
        SKRect target = VideoPresenter.ComputeDestinationRect(destination, 100, 200, VideoStretch.Uniform);
    ...
    [Fact]
    public void uniform_to_fill_covers_the_destination_and_overflows_the_long_way()
    {
        //Arrange
        SKRect destination = SKRect.Create(0f, 0f, 100f, 100f);
        ...
        //Act
        SKRect target =
            VideoPresenter.ComputeDestinationRect(destination, 200, 100, VideoStretch.UniformToFill);
    ...
    public void none_centres_the_picture_at_its_own_size()
    {
        ...
        //Act
        SKRect target = VideoPresenter.ComputeDestinationRect(destination, 40, 20, VideoStretch.None);
    ...
    public void an_empty_picture_falls_back_to_the_whole_destination()
    {
        ...
        //Act
        SKRect target = VideoPresenter.ComputeDestinationRect(destination, 0, 0, VideoStretch.Uniform);
    ...
    {
        //Arrange
        using VideoPresenter presenter = new VideoPresenter();
    ...
    [Fact]
    public void gpu_auto_falls_back_to_the_processor_and_says_so_once()
    {
        //Arrange
        using VideoPresenter presenter = new VideoPresenter { RenderPath = VideoRenderPath.GpuAuto };
    ...
    [Fact]
    public void gpu_auto_falling_back_leaves_a_configured_effect_chain_inactive()
    {
        //Arrange
        using VideoPresenter presenter = new VideoPresenter { RenderPath = VideoRenderPath.GpuAuto };
    ...
    [Fact]
    public void allow_effects_on_cpu_makes_the_chain_active_on_the_processor()
    {
        //Arrange
        using VideoPresenter presenter = new VideoPresenter { AllowEffectsOnCpu = true };
        presenter.Effects.Add(new LutEffect(Lut3D.CreateIdentity(5)));
    ...
    [Fact]
    public void gpu_no_fallback_refuses_when_there_is_no_graphics_context()
    {
        //Arrange
        using VideoPresenter presenter = new VideoPresenter
        {
        ...
        //Assert - the refusal names the setting and says what to set instead. Upstream it also names
        //the presenter's UseGpu(GRContext) call; here nothing outside the add-in can supply a context,
        //so the advice an application can actually act on is the RenderPath property.
        resolve.Should().Throw<VideoPlaybackException>()
            .WithMessage("*GpuNoFallback*")
            .WithMessage("*RenderPath to GpuAuto*");
    }
    ...
    {
        //Arrange
        using VideoPresenter presenter = new VideoPresenter { RenderPath = VideoRenderPath.Cpu };
        presenter.Effects.Add(new LutEffect(Lut3D.CreateIdentity(5)));
    ...
    // Upstream this reads three frames out of a golden ".cbv" from the engine's own test corpus. That
    // corpus is in another repository and this one adds no media, so the frames are built in code
    // instead - the same three plane layouts the corpus carries, with a pattern that varies in all
    // three planes. What is being proved is unchanged: the composition surface holds exactly the
    // bytes the engine's converter produces, to the pixel.
    [Theory]
    [InlineData(VideoPixelLayout.I420)]
    [InlineData(VideoPixelLayout.I422)]
    [InlineData(VideoPixelLayout.I444)]
    public void the_composed_surface_holds_exactly_what_the_core_converter_produces(VideoPixelLayout layout)
    {
        //Arrange
        using PinnedFrameBufferPool pool = new PinnedFrameBufferPool();
        using VideoPresenter presenter = new VideoPresenter { RenderPath = VideoRenderPath.Cpu };
        using VideoFrame frame =
            TestFrames.CreatePattern(pool, 64, 48, layout, TestFrames.Bt709Limited, 3);
    ...
    [Fact]
    public void capture_composed_frame_hands_back_an_image_the_caller_owns()
    {
        //Arrange
        using PinnedFrameBufferPool pool = new PinnedFrameBufferPool();
        using VideoPresenter presenter = new VideoPresenter { RenderPath = VideoRenderPath.Cpu };
    ...
    [Fact]
    public void current_image_is_null_until_a_frame_has_been_composed()
    {
        //Arrange
        using VideoPresenter presenter = new VideoPresenter { RenderPath = VideoRenderPath.Cpu };
    ...
    private static byte[] CaptureBytes(VideoPresenter presenter)
    {
    ...
    public void changing_the_grid_size_recomposes_the_chain_at_the_new_size()
    {
        ...
        presenter.GetResultantLut()!.Size.Should().Be(VideoPresenter.DefaultEffectLutSize);
