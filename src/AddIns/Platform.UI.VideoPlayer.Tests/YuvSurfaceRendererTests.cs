...
public class YuvSurfaceRendererTests
{
    /// <summary>
    ...
    public YuvSurfaceRendererTests(ITestOutputHelper output) => this.output = output;
    ...
    // Upstream this decodes six frames out of a golden ".cbv" from the engine's own test corpus. That
    // corpus is in another repository and this one adds no media, so six frames are built in code
    // instead, each with a different pattern seed - the point of the test is that the shader tracks
    // the converter over a SEQUENCE of differing pictures rather than one lucky frame.
    [Fact]
    public void the_shader_agrees_with_the_core_converter_over_a_sequence_of_frames()
    {
        //Arrange
        using PinnedFrameBufferPool pool = new PinnedFrameBufferPool();
        List<VideoFrame> frames = new List<VideoFrame>(6);
        for (int seed = 0; seed < 6; seed++)
        {
            frames.Add(TestFrames.CreatePattern(
                pool,
                64,
                48,
                ...
                seed,
                TimeSpan.FromMilliseconds(seed * 40),
                seed));
        }
    ...
    public void an_identity_atlas_in_the_shader_leaves_the_picture_where_it_was()
    {
        ...
        using SKBitmap atlas = BuildAtlas(new EffectComposer(33));
    ...
    private static unsafe SKBitmap BuildAtlas(EffectComposer composer)
    {
