...
//process has no head, so this installs inert ones. Copied from the SkiaSharp.Views add-in's suite
//(src/AddIns/CodeBrix.Platform.SkiaSharp.Views.Tests/DispatcherInitializer.cs), namespace changed.
...
/// <summary>
/// Installs host-free dispatcher overrides so the add-in's XAML controls (ToolBar, ToolButton and
/// the rest, all dependency objects) can be constructed in a test process with no application
/// head.
/// </summary>
/// <remarks>
/// <para>
/// Every thread reports that it has dispatcher access, and dispatched work runs inline. That is
/// the right semantic for unit tests, which are synchronous and assert on immediate effects - a
/// property set in a test is seen by the control's changed handler before the setter returns,
/// rather than on some later dispatcher turn that never comes.
/// </para>
/// </remarks>
