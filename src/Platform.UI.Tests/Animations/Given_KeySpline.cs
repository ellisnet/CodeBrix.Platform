...
namespace CodeBrix.Platform.UI.Tests.Animations //Was previously: Uno.UI.Tests.Animations
{
	...
	public class Given_KeySpline
	{
		...
		public void When_Matching(double x1, double y1, double x2, double y2)
		{
			...
			var codebrixResults = Compute(uno.GetSplineProgress);

			string r1String = string.Join(" ", originalResults.Select(x => x.ToString(".000")));
			string r2String = string.Join(" ", codebrixResults.Select(x => x.ToString(".000")));
			...
			for (int i = 0; i < originalResults.Count - 1; i++)
			{
				...
				Assert.AreEqual(originalResults[i], codebrixResults[i], 0.005);
			}

			Assert.AreEqual(1.0, codebrixResults[originalResults.Count - 1], 0.00005);
		}
