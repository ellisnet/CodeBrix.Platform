...
namespace CodeBrix.Platform.UI.Tests.Windows_Globalization //Was previously: Uno.UI.Tests.Windows_Globalization
{
	[TestClass]
	public class Given_PercentFormatter
	{
		...
		// In UWP PercentFormatter() ignore PrimaryLanguageOverride
		// and use the localization settings of the OS;
		// to avoid this you need to use the constructor
		// public PercentFormatter([In] IEnumerable<string> languages, [In] string geographicRegion).
		private static PercentFormatter MakeFormatter() =>
#if HAS_CODEBRIX || IS_UNIT_TESTS
			new PercentFormatter();
#else
			new PercentFormatter(new[] { "en-us" }, "US");
#endif
	}
}
