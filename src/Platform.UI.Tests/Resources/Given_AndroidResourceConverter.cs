...
namespace CodeBrix.Platform.UI.Tests.Resources //Was previously: Uno.UI.Tests.Resources
{
	[TestClass]
	public class Given_AndroidResourceConverter
	{
		...
		{
			// Test data is authored with Windows path separators; align to the running OS so the
			// expectation matches the converter output on Windows (no-op) and Linux/macOS ('/').
			expectedPath = expectedPath?.Replace('\\', Path.DirectorySeparatorChar);
			logicalPath = logicalPath?.Replace('\\', Path.DirectorySeparatorChar);
	...
	public class Given_AndroidResourceNameEncoder
	{
		...
		{
			// Test data is authored with Windows path separators; align to the running OS so the
			// expectation matches the encoder output on Windows (no-op) and Linux/macOS ('/').
