...
#if false //TODO:MZ: Should be mac catalyst specific?
...
#else
		var focusInputHandler = CodeBrix.Platform.UI.Xaml.Core.CoreServices.Instance.MainRootVisual?.AssociatedVisualTree?.CodeBrixFocusInputHandler;
		if (CodeBrix.Platform.WinRTFeatureConfiguration.Focus.EnableExperimentalKeyboardFocus && focusInputHandler != null)
		{
			foreach (UIPress press in presses)
			{
				if (press.Key is null)
