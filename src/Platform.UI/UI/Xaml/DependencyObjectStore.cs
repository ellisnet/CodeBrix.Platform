...
namespace Microsoft.UI.Xaml
{
	...
	public partial class DependencyObjectStore : IDisposable
	{
		...
		{
#if DEBUG
			var hashSet = new HashSet<DependencyObject>(CodeBrix.Platform.Extensions.ReferenceEqualityComparer<DependencyObject>.Default);
			if (instance != null)
			{
		...
		)
		{
			...
			if (AreDifferent(newValue, previousValue))
			{
				...
				&& _propagationBypassed.TryGetValue(propertyPath!, out var unpropagatedPrevious)
			...
			else if (this.Log().IsEnabled(CodeBrix.Platform.Foundation.Logging.LogLevel.Debug))
			{
				this.Log().Debug(
		...
		private static DependencyPropertyChangedEventArgsPool _dpChangedEventArgsPool =
			new(CodeBrix.Platform.UI.FeatureConfiguration.DependencyProperty.DependencyPropertyChangedEventArgsPoolSize);
		...
		private ManagedWeakReference ThisWeakReference
			=> _thisWeakRef ??= CodeBrix.Platform.UI.DataBinding.WeakReferencePool.RentWeakReference(this, this);
