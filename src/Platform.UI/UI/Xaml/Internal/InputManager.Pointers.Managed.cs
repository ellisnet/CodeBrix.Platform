...
internal partial class InputManager
{
	...
	internal partial class PointerManager
	{
		...
		private ICodeBrixCorePointerInputSource? _source;

		// ONLY USE THIS FOR TESTING PURPOSES
		internal ICodeBrixCorePointerInputSource? PointerInputSourceForTestingOnly => _source;
		...
		public void Init(object host)
		{
			if (!ApiExtensibility.CreateInstance(host, out _source))
			{
				...
				{
					this.Log().Error("Failed to initialize the PointerManager: cannot resolve the ICodeBrixCorePointerInputSource.");
				}
				return;
			}
