	...
	public partial class UIElement
	{
		...
		internal object[] GetKeysForResource(object resource)
		{
			...
			IEnumerable<object> Inner()
			{
				...
				foreach (var key in TryFindResource(CodeBrix.Platform.UI.GlobalStaticResources.MasterDictionary))
				{
					yield return key;
				}
		...
		internal FrameworkElement GetPropInheritanceParent(string propertyName)
		{
			...
			return CodeBrix.Platform.UI.Extensions.DependencyObjectExtensions.FindFirstParent<FrameworkElement>(this, fe => IsInherited(GetDPByName(propertyName, fe), fe.GetType()), includeCurrent: false);
		...
		internal string GetVisualStatesSummary()
		{
			...
			foreach (var ancestor in CodeBrix.Platform.UI.Extensions.DependencyObjectExtensions.GetAllParents(this))
			{
