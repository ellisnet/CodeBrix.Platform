...
#pragma warning restore 414
		...
		private Rect _codebrix_viewportUsedInLastMeasure;
#endif
...
#if !CODEBRIX_HAS_ENHANCED_LIFECYCLE
			// Uno workaround: Perf
			_codebrix_viewportUsedInLastMeasure = m_visibleWindow;
#endif
		}
		...
		void UpdateViewport(Rect viewport)
		{
			...
			if (m_owner.Layout is VirtualizingLayout vl // If not a VirtualizingLayout, we actually don't have to re-measure items!
				&& vl.IsSignificantViewportChange(m_owner.LayoutState, _codebrix_viewportUsedInLastMeasure, m_visibleWindow))
			// Uno workaround [END]
#endif
			{
				TryInvalidateMeasure();
