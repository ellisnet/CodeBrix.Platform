...
namespace Microsoft.UI.Xaml.Controls
{
	public partial class StackLayout : VirtualizingLayout, IFlowLayoutAlgorithmDelegates
	{
		...
		protected internal override Size MeasureOverride(VirtualizingLayoutContext context, Size availableSize)
		{
			...
			state.Platform_LastKnownItemsCount = context.ItemCount;
			state.Platform_LastKnownRealizedElementsCount = algo.RealizedElementCount;
			state.Platform_LastKnownDesiredSize = desiredSize;
			// Uno workaround [END]
			...
			StackLayoutState stackLayoutState)
		{
			...
			stackLayoutState.Platform_LastKnownAverageElementSize = averageElementSize;
		...
		{
			if (state is StackLayoutState { Platform_LastKnownAverageElementSize: > 0 } stackState)
			{
				var neededElementsCountToFillNewViewport = Math.Min(stackState.Platform_LastKnownItemsCount, MajorSize(newViewport) / stackState.Platform_LastKnownAverageElementSize);
				if (stackState.Platform_LastKnownRealizedElementsCount < neededElementsCountToFillNewViewport)
				{
					...
					var isOutOfBounds = newViewport is { Width: 0 } or { Height: 0 }
						...
						|| MajorStart(newViewport) > Major(stackState.Platform_LastKnownDesiredSize) + threshold
						|| MinorEnd(newViewport) < -threshold
						|| MinorStart(newViewport) > Minor(stackState.Platform_LastKnownDesiredSize) + threshold;

					return !isOutOfBounds;
				}

				var size = Math.Max(MajorSize(oldViewport), MajorSize(newViewport));
				var minDelta = Math.Min(stackState.Platform_LastKnownAverageElementSize * 5, size);
