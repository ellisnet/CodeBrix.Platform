...
namespace Microsoft.UI.Xaml.Controls
{
	...
	public partial class ItemsRepeater : FrameworkElement, IPanel
	{
		...
		protected override Size MeasureOverride(Size availableSize)
		{
			...
			using var layoutInProgress = CodeBrix.Platform.Extensions.Disposables.Disposable.Create(() => m_isLayoutInProgress = false);
		...
		{
#if !HAS_CODEBRIX // With uno we always detach from the scroller on unload so we need to force a layouting pass to re-subscribe to scroller and update viewport (in a single pass)
...
#if !HAS_CODEBRIX // Avoids leak and useless as we are not validating such count in the loaded
		...
		{
			if (CodeBrix.Platform.UI.TemplateManager.IsDataTemplateDynamicUpdateEnabled)
			{
		...
		internal event TypedEventHandler<ItemsRepeater, ItemsRepeaterElementPreparedEventArgs> CodeBrixBeforeElementPrepared;

		internal void OnCodeBrixBeforeElementPrepared(UIElement element, int index)
		{
			var args = new ItemsRepeaterElementPreparedEventArgs(element, index);
			CodeBrixBeforeElementPrepared?.Invoke(this, args);
		}
