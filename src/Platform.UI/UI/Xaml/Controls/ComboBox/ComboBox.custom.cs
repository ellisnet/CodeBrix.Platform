...
public partial class ComboBox : Selector
{
	...
	protected override void OnApplyTemplate()
	{
		...
		{
			////TODO CodeBrix specific: Ensures popup does not take focus when opened.
...
#pragma warning restore CS0162 // Unreachable code detected
		...
		public void Arrange(Size finalSize, Rect visibleBounds, Size desiredSize)
		{
			...
			var codebrixPlacement = CodeBrix.Platform.UI.Xaml.Controls.ComboBox.GetDropDownPreferredPlacement(combo);
			var winUIPlacement = popup.DesiredPlacement;

			if (combo.IsEditable)
			{
				codebrixPlacement = DropDownPlacement.Below;

				if (comboRect.Bottom + frame.Height > visibleBounds.Bottom)
				{
					codebrixPlacement = DropDownPlacement.Above;
				}
			}

			if (codebrixPlacement == DropDownPlacement.Auto)
			{
				// If the Uno placement is Auto, we use the WinUI placement
				codebrixPlacement = winUIPlacement switch
				{
