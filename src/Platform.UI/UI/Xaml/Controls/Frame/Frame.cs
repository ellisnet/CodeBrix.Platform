...
public partial class Frame : ContentControl
{
		...
		Type sourcePageType)
	{
		...
		if (CodeBrix.Platform.UI.DataBinding.BindingPropertyHelper.BindableMetadataProvider != null)
		{
			var bindableType = CodeBrix.Platform.UI.DataBinding.BindingPropertyHelper.BindableMetadataProvider.GetBindableTypeByType(replacementType);
	...
	{
#if HAS_CODEBRIX // This workaround is currently needed to handle focus in line with WinUI (and is required even for enhanced lifecycle targets for now).
	...
	/// but due to the control lifecycle differences in CodeBrix the focus move multiple times
