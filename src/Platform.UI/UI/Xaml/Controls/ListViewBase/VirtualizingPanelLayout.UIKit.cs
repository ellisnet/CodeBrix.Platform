...
using _LayoutAttributes = Microsoft.UI.Xaml.Controls.CodeBrixUICollectionViewLayoutAttributes;
using LayoutInfoDictionary = System.Collections.Generic.Dictionary<Foundation.NSIndexPath, Microsoft.UI.Xaml.Controls.CodeBrixUICollectionViewLayoutAttributes>;
#else
...
{
#if USE_CUSTOM_LAYOUT_ATTRIBUTES
	public class CodeBrixUICollectionViewLayoutAttributes : UICollectionViewLayoutAttributes
	{
		/// <inheritdoc />
	...
	public abstract partial class VirtualizingPanelLayout : UICollectionViewLayout, DependencyObject
	{
		...
		{
			return _itemLayoutInfos.PlatformGetValueOrDefault(indexPath.Section)?.PlatformGetValueOrDefault(indexPath)!;
		}
		...
		public override UICollectionViewLayoutAttributes LayoutAttributesForSupplementaryView(NSString kind, NSIndexPath indexPath)
		{
			...
			var layoutAttributes = _supplementaryLayoutInfos.PlatformGetValueOrDefault(kind)?.PlatformGetValueOrDefault(indexPath);
		...
		public override UICollectionViewLayoutAttributes InitialLayoutAttributesForAppearingItem(NSIndexPath itemIndexPath)
		{
			...
			if (!isUpdatingItem)
			{
				var codebrixCachedAttributes = LayoutAttributesForItem(itemIndexPath);
				if (codebrixCachedAttributes != null)
				{
					attributes.Frame = codebrixCachedAttributes.Frame;
				}
			}
		...
		public override UICollectionViewLayoutAttributes FinalLayoutAttributesForDisappearingItem(NSIndexPath itemIndexPath)
		{
			...
			{
				var codebrixCachedAttributes = LayoutAttributesForItem(itemIndexPath);
				if (codebrixCachedAttributes is not null && attributes is not null)
				{
					attributes.Frame = codebrixCachedAttributes.Frame;
				}
			}
		...
		private CGSize PrepareLayoutInternal(bool createLayoutInfo, bool isCollectionChanged, CGSize size)
		{
			...
			var oldHeaderSize = _supplementaryLayoutInfos
				.PlatformGetValueOrDefault(NativeListViewBase.ListViewHeaderElementKind)?
				.FirstOrDefault()
				.Value?.Frame.Size;
			var oldFooterSize = _supplementaryLayoutInfos
				.PlatformGetValueOrDefault(NativeListViewBase.ListViewFooterElementKind)?
				.FirstOrDefault()
				.Value?.Frame.Size;
			...
			if (isCollectionChanged)
			{
				...
				oldGroupHeaderSizes = _supplementaryLayoutInfos
					.PlatformGetValueOrDefault(NativeListViewBase.ListViewSectionHeaderElementKind)?
					.Select(kvp =>
					{
						return
						(
			...
			for (int section = 0; section < sections; section++)
			{
				...
				if (isGrouping)
				{
					...
					frame.Size = oldGroupHeaderSizes?.PlatformGetValueOrDefault(section) ?? GetSectionHeaderSize(section, size);
			...
			{
				var newAnchor = _itemLayoutInfos.PlatformGetValueOrDefault(indexPath.Section)?.PlatformGetValueOrDefault(indexPath);
				if (newAnchor == null)
				{
		...
		private void UpdateHeaderPositions()
		{
			...
			if (_supplementaryLayoutInfos.TryGetValue(NativeListViewBase.ListViewSectionHeaderElementKind, out var headerAttributes))
			{
				foreach (var kvp in headerAttributes)
				{
					...
					if (this.Log().IsEnabled(CodeBrix.Platform.Foundation.Logging.LogLevel.Debug) && frameOffset != (axisIndex == 0 ? frame.X : frame.Y))
					{
		...
		public void UpdateLayoutAttributesForElement(_LayoutAttributes layoutAttributes)
		{
			...
			{
				var sectionHeaderLayouts = _supplementaryLayoutInfos.PlatformGetValueOrDefault(NativeListViewBase.ListViewSectionHeaderElementKind);
				...
				{
					var layout = _supplementaryLayoutInfos.PlatformGetValueOrDefault(NativeListViewBase.ListViewFooterElementKind)?.PlatformGetValueOrDefault(path);
		...
		private CodeBrix.Platform.UI.IndexPath GetFirstVisibleIndexPath()
		{
			return CollectionView.IndexPathsForVisibleItems
				.OrderBy(p => p.ToIndexPath())
				.FirstOrDefault()?
				.ToIndexPath()
					?? CodeBrix.Platform.UI.IndexPath.FromRowSection(-1, 0);
		}
		...
		private protected CodeBrix.Platform.UI.IndexPath? GetAndUpdateReorderingIndex() => throw new NotSupportedException("Not used on iOS");
