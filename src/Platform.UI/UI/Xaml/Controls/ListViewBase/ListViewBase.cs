...
namespace Microsoft.UI.Xaml.Controls
{
	public partial class ListViewBase : Selector
	{
		...
		private void OnSelectedItemsCollectionChanged(object sender, NotifyCollectionChangedEventArgs e)
		{
			...
			var items = GetItems();
			if (items == null && _oldSelectedItems.Count != 0)
			{
			...
			if (validAdditions.Length != 0 || validRemovals.Length != 0)
			{
		...
		private void ReplaceItems(int firstItem, int count, int section)
		{
			...
			{
				var codebrixIndexPath = CodeBrix.Platform.UI.IndexPath.FromRowSection(firstItem + i, section);
				var flatIndex = GetIndexFromIndexPath(codebrixIndexPath);
				var container = ContainerFromIndex(flatIndex);
				if (container != null)
				{
#if false // TODO: The managed ListView should similarly go through the recycling to use the proper container matching the new template
					var item = GetDisplayItemFromIndexPath(codebrixIndexPath);
