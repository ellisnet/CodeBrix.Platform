...
namespace Microsoft.UI.Xaml.Controls
{
	public partial class NativeListViewBase : UICollectionView
	{
		...
		/// </summary>
		public bool AnimateScrollIntoView { get; set; } = CodeBrix.Platform.UI.FeatureConfiguration.ListViewBase.AnimateScrollIntoView;
		#endregion
		...
		public override void InsertSections(NSIndexSet sections)
		{
			if (TryApplyCollectionChange())
			{
				...
				{
					NativeLayout?.NotifyCollectionChange(new CollectionChangedOperation(
									CodeBrix.Platform.UI.IndexPath.FromRowSection(0, (int)sections.FirstIndex),
									(int)sections.Count,
									NotifyCollectionChangedAction.Add,
		...
		public override void LayoutSubviews()
		{
			...
			{
				var cell = tuple.Cell;
				var codebrixCell = cell as ListViewBaseInternalContainer;

				if (codebrixCell == null)
				{
					continue;
				}

				var isAnimating = codebrixCell./* cache */Layer.AnimationKeys?.Any() ?? false;
				if (isAnimating)
				{
				...
				var expectedItem = XamlParent?.
					ItemFromIndex(XamlParent.
						GetIndexFromIndexPath(CodeBrix.Platform.UI.IndexPath.FromNSIndexPath(tuple.Path)
					)
				);
				var actualItem = codebrixCell.Content?.DataContext;
					...
					&& XamlParent.DisplayMemberPath.IsNullOrEmpty())
				{
					...
					if (!areMatching)
					{
						...
						{
							(this).Log().Warn($"Cell had context {actualItem} instead of {expectedItem}, scheduling a refresh to ensure correct display of list. (CodeBrix.Platform.UI.IndexPath={tuple.Path}");
						}
						DispatchReloadData();
					}
				}
