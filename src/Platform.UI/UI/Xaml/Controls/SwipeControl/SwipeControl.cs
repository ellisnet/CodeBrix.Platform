	...
	public partial class SwipeControl : ContentControl
	{
		...
		private void SwipeControl_Unfinalizer()
		{
			if (_isReady)
			{
				...
				AttachEventHandlers(isCodeBrixUnfinalizer: true);
			}
		}
#endif
		...
		private void AttachEventHandlers(bool isCodeBrixUnfinalizer = false)
		{
			SWIPECONTROL_TRACE_INFO(this/*, TRACE_MSG_METH, METH_NAME, this*/);

			//global::System.Diagnostics.Debug.Assert(m_loadedToken.value == 0);
			if (isCodeBrixUnfinalizer) // Uno workaround: We detach from event on unload and re-attach on loaded
			{
				OnLoaded(this, null);
			}
			else
