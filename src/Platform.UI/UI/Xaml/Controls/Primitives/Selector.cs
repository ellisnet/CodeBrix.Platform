	...
	public partial class Selector : ItemsControl
	{
		...
		private void UpdateSelectedValue()
		{
			if (!SelectedValuePath.IsNullOrEmpty())
			{
				...
				{
					_selectedValueBindingPath = new CodeBrix.Platform.UI.DataBinding.BindingPath(SelectedValuePath, null);
				}
			}
			else
			{
		...
		/// <summary>
		/// The selected index as an <see cref="CodeBrix.Platform.UI.IndexPath"/> of (group, group position), where group=0 if the source is ungrouped.
		/// </summary>
		private CodeBrix.Platform.UI.IndexPath? SelectedIndexPath { get; set; }
		...
		internal override void OnItemsSourceSingleCollectionChanged(object sender, NotifyCollectionChangedEventArgs c, int section)
		{
			...
			switch (c.Action)
			{
				case NotifyCollectionChangedAction.Add:
					...
					var newIndex = GetIndexFromIndexPath(CodeBrix.Platform.UI.IndexPath.FromRowSection(c.NewStartingIndex, section));
				...
				case NotifyCollectionChangedAction.Remove:
					{
						var oldIndex = GetIndexFromIndexPath(CodeBrix.Platform.UI.IndexPath.FromRowSection(c.OldStartingIndex, section));
		...
		internal override void OnItemsSourceGroupsChanged(object sender, NotifyCollectionChangedEventArgs c)
		{
			...
			{
				case NotifyCollectionChangedAction.Add:
					var newIndexPath = CodeBrix.Platform.UI.IndexPath.FromRowSection(row: 0, section: c.NewStartingIndex);
				...
				case NotifyCollectionChangedAction.Remove:
					{
						var oldIndexPath = CodeBrix.Platform.UI.IndexPath.FromRowSection(row: 0, section: c.OldStartingIndex);
						var oldIndexPathLast = CodeBrix.Platform.UI.IndexPath.FromRowSection(row: int.MaxValue, section: c.OldStartingIndex + c.OldItems.Count);
