...
#define CODEBRIX_USE_DEFERRED_VISUAL_STATES
#endif
...
public partial class NavigationViewItem : NavigationViewItemBase
{
	...
	private void UpdateVisualStateForPointer()
	{
		...
		string GetSelectedStateValue(bool isEnabled, bool isSelected)
		{
			if (isEnabled)
			{
				if (isSelected)
				{
					if (m_isPressed && !_codebrix_isDefferingPressedState)
					{
						return c_pressedSelected;
					}
					else if (m_isPointerOver && !_codebrix_isDefferingOverState)
					{
				...
				else if (m_isPressed && !_codebrix_isDefferingPressedState)
				{
					return c_pressed;
				}
			}
...
#if CODEBRIX_USE_DEFERRED_VISUAL_STATES
		_codebrix_isDefferingPressedState = true;
	...
	{
#if CODEBRIX_USE_DEFERRED_VISUAL_STATES
		_codebrix_isDefferingOverState = args.Pointer.PointerDeviceType != PointerDeviceType.Mouse;
		DeferUpdateVisualStateForPointer();
#endif
	...
	private void ProcessPointerCanceled(PointerRoutedEventArgs args)
	{
		...
		_codebrix_pointerDeferring?.Stop();
