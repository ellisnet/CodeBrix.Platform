...
#if false // On wasm the scroll might be async (especially with disableAnimation: false), so we need to use the pending value to support high speed multiple scrolling events
...
#elif false // Note: No __ANDROID__, the ICustomScrollInfo support is made directly in the NativeScrollContentPresenter
