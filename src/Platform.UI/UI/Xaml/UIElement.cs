...
#nullable enable
		internal static Matrix3x2 GetTransform(UIElement from, UIElement? to)
		{
			...
			return from2To;
#else
#if CODEBRIX_REFERENCE_API // Depth is defined properly only on WASM and Skia
...
#pragma warning restore CS0649
...
#if false // IsMeasureDirty and IsArrangeDirty are not available on iOS / macOS
		...
		internal static string SetDependencyPropertyValueInternal(DependencyObject owner, string dependencyPropertyNameAndValue)
		{
			...
			if (index != -1)
			{
				...
				{
					if (owner.Log().IsEnabled(CodeBrix.Platform.Foundation.Logging.LogLevel.Debug))
					{
		...
		/// <remarks>
		/// Under CodeBrix.Platform.UI, this method should not be called during the normal layouting phase. Instead, use the
