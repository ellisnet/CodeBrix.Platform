...
public partial class LinearGradientBrush
{
	...
	{
#if false // On WASM, linear gradient borders work only if there is no CornerRadius applied to the control
		return cornerRadius == CornerRadius.None;
#elif false // On iOS and macOS, we can apply linear gradient borders reliably only when there is no RelativeTransform applied
