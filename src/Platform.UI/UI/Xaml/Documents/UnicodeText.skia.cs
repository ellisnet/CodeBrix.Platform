...
internal readonly partial struct UnicodeText : IParsedText
{
	...
	private readonly record struct CodeBrixGlyphPosition(GlyphPosition GlyphPosition, float XAdvance);
	...
	private readonly record struct ShapedLineBrokenBidiRun(ReadonlyInlineCopy Inline, int startInInline, int endInInline, (GlyphInfo info, CodeBrixGlyphPosition position)[] glyphs, FontDetails fontDetails, bool rtl);
	private record LayoutedGlyphDetails(GlyphInfo info, CodeBrixGlyphPosition position, float xPosInRun, Cluster? cluster, LayoutedLineBrokenBidiRun parentRun)
	{
	...
	private static (GlyphInfo info, CodeBrixGlyphPosition position)[] ShapeRun(string textRun, bool rtl, FontDetails fontDetails, float? currentLineWidth, bool ignoreTrailingSpaces)
	{
		...
		var ret = new (GlyphInfo, CodeBrixGlyphPosition)[count];
		for (var i = 0; i < count; i++)
		{
			ret[i] = (infos[i], new CodeBrixGlyphPosition(positions[i], positions[i].XAdvance));
		}
		...
		if ((isTab || IsLineBreak(textRun, textRun.Length)) && infos[^1].Cluster == textRun.Length - (textRun is [.., '\r', '\n'] ? 2 : 1))
		{
			...
			ret[^1] = (infos[^1] with { Codepoint = spaceCodepoint }, new CodeBrixGlyphPosition(positions[^1] with { XAdvance = (int)xAdvance }, xAdvance));
		}
		...
		if (ret is [])
		{
			...
			fontDetails.Font.TryGetGlyph(' ', out var spaceCodepoint);
			ret = [(new GlyphInfo { Codepoint = spaceCodepoint }, new CodeBrixGlyphPosition(new GlyphPosition(), 0))];
		}
		return ret;
	}
