...
namespace Microsoft.UI.Xaml
{
	...
	{
		private const bool CanUseTypeGetType =
#if false
			// Workaround for https://github.com/dotnet/runtime/issues/45078
			Environment.GetEnvironmentVariable("CODEBRIX_BOOTSTRAP_MONO_RUNTIME_MODE") == "Interpreter";
#else
			true;
#endif
			...
			if (propertyPath.Contains(':'))
			{
				...
				if (bindingParts.Length == 2)
				{
					...
					if (propertyParts.Length == 2)
					{
						...
						var type = CodeBrix.Platform.UI.DataBinding.BindingPropertyHelper
					...
					{
						if (typeof(DependencyPropertyDescriptor).Log().IsEnabled(CodeBrix.Platform.Foundation.Logging.LogLevel.Debug))
						{
