...
#pragma warning restore 114 // Error CS0114: 'FrameworkElement.Tag' hides inherited member 'UIView.Tag'
		...
		public event EventHandler<object> LayoutUpdated
		{
			add
			{
				...
				{
					CodeBrix.Platform.UI.Extensions.DependencyObjectExtensions.GetContext(this).EventManager.AddLayoutUpdatedEventHandler(this);
				}
#endif
			}
			remove
			{
				...
				{
					CodeBrix.Platform.UI.Extensions.DependencyObjectExtensions.GetContext(this).EventManager.RemoveLayoutUpdatedEventHandler(this);
				}
#endif
			}
		}
		...
		internal static void RegisterPhaseBinding(ContentControl content, Action<Action> registerForRecycled)
		{
			...
			if (presenterRoot != null)
			{
				...
				{
					CodeBrix.Platform.UI.Dispatching.UIAsyncOperation action = null;
					var phaseCapture = i;
					...
					action = (CodeBrix.Platform.UI.Dispatching.UIAsyncOperation)presenterRoot.Dispatcher.RunAnimation(ApplyPhase);
#elif false
					action = (CodeBrix.Platform.UI.Dispatching.UIAsyncOperation)presenterRoot.Dispatcher.RunAsync(CoreDispatcherPriority.High, ApplyPhase);
#endif
...
#if true // This code is generated in FrameworkElementMixins
