	...
	internal class XamlTypeResolver
	{
		...
		private Type? SourceFindType(string? name)
		{
			...
			else
			{
				...
				var clrNamespaces = KnownNamespaces.PlatformGetValueOrDefault(defaultXmlNamespace, Array.Empty<string>());
		...
		private Type? SourceFindPropertyType(XamlMember xamlMember)
		{
			...
			{
				var clrNamespaces = KnownNamespaces.PlatformGetValueOrDefault(xamlMember.DeclaringType.PreferredXamlNamespace, Array.Empty<string>());
