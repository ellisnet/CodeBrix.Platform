...
using CodeBrix.Platform.UI.Helpers.Xaml;
using CodeBrix.Platform.UI.Xaml;
using CodeBrix.Platform.UI.Xaml.Markup;
...
namespace Microsoft.UI.Xaml.Markup.Reader
{
	...
	internal partial class XamlObjectBuilder
	{
			...
			bool createInstanceFromXClass = false)
		{
			...
			{
				var generated = CodeBrix.Platform.Media.Parsers.ParseGeometry(geometryStringValue);
					...
					if (instance is Style style)
					{
						if (control.Members.FirstOrDefault(m => m.Member.Name == "TargetType") is { } targetTypeDefinition)
						{
							if (BuildLiteralValue(targetTypeDefinition) is Type targetType)
							{
								...
								return CodeBrix.Platform.Extensions.Disposables.Disposable.Create(() =>
								{
			...
			TemplateMaterializationSettings? settings)
		{
			...
			{
				[UnconditionalSuppressMessage("Trimming", "IL3050", Justification = "Hopefully CodeBrix.Platform.UI.SourceGenerators.BindableTypeProviders.BindableTypeProvidersSourceGenerator will preserve what is needed.")]
		...
		private object? BuildLiteralValue(Type propertyType, string? memberValue)
		{
			...
			return CodeBrix.Platform.UI.DataBinding.BindingPropertyHelper.Convert(propertyType, memberValue);
		}
