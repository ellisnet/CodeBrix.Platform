...
using CollectionExtensions = CodeBrix.Platform.Extensions.CollectionExtensions;
	...
	public partial class GestureRecognizer
	{
		...
		internal partial class Manipulation
		{
			...
			private ManipulationDelta ComputeDelta(in PointsState from, in PointsState to, in ManipulationDelta previous)
			{
				...
				{
					rotation = _isRotateEnabled ? CodeBrix.Platform.Extensions.MathEx.ToDegree(to.Angle - from.Angle) : 0;
			...
			{
				if (OperatingSystem.IsAndroid() && _recognizer.PatchCases.HasFlag(CodeBrix.Platform.UI.Input.GestureRecognizerSuspiciousCases.AndroidMotionUpAtInvalidLocation))
				{
				...
				private PointsState ComputeState(ulong timestamp)
				{
					...
					else
					{
						...
						return new(
							...
							Distance: Vector2.Distance(global::System.Numerics.VectorExtensions.ToVector2(p1), global::System.Numerics.VectorExtensions.ToVector2(p2)),
