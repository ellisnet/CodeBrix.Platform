...
public sealed partial class BrowserHtmlElement : IDisposable
{
	...
	internal nint CodeBrixElementId { get; }
	...
	private BrowserHtmlElement(bool isOwner)
	{
		...
		CodeBrixElementId = handle;
		ElementId = "CodeBrix-" + handle;
		IsOwner = isOwner;
	}
	...
	/// instance to be managed by CodeBrix's native element hosting logic. After this call,
	...
	public static BrowserHtmlElement CreateHtmlElement(string elementId, string tagName)
	{
		...
		CreateHtmlElementNative(element.ElementId, element.CodeBrixElementId, tagName);
		return element;
	}
	...
	/// Don't use this when there's child elements managed by CodeBrix or you'll
