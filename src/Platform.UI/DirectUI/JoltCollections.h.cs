...
namespace DirectUI
{
	...
	internal class CodeBrixVectorViewToEnumerableAdapter<T> : IEnumerator<T>
	{
		private readonly IVectorView<T> _view;
		private int _index = -1;

		public CodeBrixVectorViewToEnumerableAdapter(IVectorView<T> view) => _view = view;
	...
	internal class CodeBrixEnumeratorToIteratorAdapter<T> : IIterator<T>
	{
		private readonly IEnumerator<T> _inner;

		public CodeBrixEnumeratorToIteratorAdapter(IEnumerator<T> inner)
		{
			_inner = inner;
		}
	...
	internal interface IVectorView<T> : IReadOnlyList<T>
	{
		...
		public IEnumerator<T> GetEnumerator() => new CodeBrixVectorViewToEnumerableAdapter<T>(this);
		...
		IIterable<T>
	{
		...
		IEnumerator IEnumerable.GetEnumerator() => GetEnumerator();
		public IIterator<T> GetIterator() => new CodeBrixEnumeratorToIteratorAdapter<T>(GetEnumerator());
