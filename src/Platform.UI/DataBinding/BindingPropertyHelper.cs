...
namespace CodeBrix.Platform.UI.DataBinding //Was previously: Uno.UI.DataBinding
{
	...
	internal static partial class BindingPropertyHelper
	{
		...
		private static Type? _codebrixGetMemberBindingType;
		private static Type? _codebrixSetMemberBindingType;
		...
		private static ValueGetterHandler InternalGetValueGetter(Type type, string property, bool allowPrivateMembers)
		{
...
#endif
			{
				...
				{
					return [DynamicDependency(DynamicallyAccessedMemberTypes.All, typeof(CodeBrixGetMemberBinder))] (instance) =>
					{
						if (__LinkerHints.Is_System_Dynamic_DynamicObject_Available
							&& instance is System.Dynamic.DynamicObject dynamicObject)
						{
							// Referencing CodeBrixGetMemberBinder using a typeof or a generic type is enough to pull
							...
							_codebrixGetMemberBindingType ??=
								Type.GetType("CodeBrix.Platform.UI.DataBinding.BindingPropertyHelper+CodeBrixGetMemberBinder, " + typeof(BindingPropertyHelper).Assembly.FullName)
								?? throw new InvalidOperationException();

							var binder = (GetMemberBinder?)Activator.CreateInstance(_codebrixGetMemberBindingType, property, true);
							...
							{
								_log.ErrorFormat("The type CodeBrixGetMemberBinder is not available, likely caused by an incorrect Linker configuration.");
							}
						}
		...
		private static ValueSetterHandler InternalGetValueSetter(Type type, string property, bool convert, DependencyPropertyValuePrecedences precedence)
		{
			...
			if (IsIndexerFormat(property))
			{
				...
				var indexerInfo = CodeBrix.Platform.Extensions.Funcs.CreateMemoized(() =>
				{
				...
				var indexerType = CodeBrix.Platform.Extensions.Funcs.CreateMemoized(() => indexerInfo()?.PropertyType);
...
#endif
			{
				...
				if (propertyInfo != null)
				{
					...
					if (setMethod != null)
					{
						var propertyType = CodeBrix.Platform.Extensions.Funcs.CreateMemoized(() => GetPropertyOrDependencyPropertyType(type, property));
				...
				if (__LinkerHints.Is_System_Dynamic_DynamicObject_Available
					&& type.Is(typeof(System.Dynamic.DynamicObject)))
				{
					return [DynamicDependency(DynamicallyAccessedMemberTypes.All, typeof(CodeBrixSetMemberBinder))] (instance, value) =>
					{
						if (__LinkerHints.Is_System_Dynamic_DynamicObject_Available
							&& instance is System.Dynamic.DynamicObject dynamicObject)
						{
							// Referencing CodeBrixSetMemberBinder using a typeof or a generic type is enough to pull
							// the System.Dynamic containing assembly. We're using reflection to create the binder
							// given that the full type has been kept through `DynamicDependency` provided above.
							_codebrixSetMemberBindingType ??=
								Type.GetType("CodeBrix.Platform.UI.DataBinding.BindingPropertyHelper+CodeBrixSetMemberBinder, " + typeof(BindingPropertyHelper).Assembly.FullName)
								?? throw new InvalidOperationException();

							var binder = (SetMemberBinder?)Activator.CreateInstance(_codebrixSetMemberBindingType, property, true);
							...
							{
								_log.ErrorFormat("The type CodeBrixSetMemberBinder is not available, likely caused by an incorrect Linker configuration.");
							}
