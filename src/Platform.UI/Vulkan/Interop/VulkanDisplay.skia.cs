...
namespace CodeBrix.Platform.UI.Runtime.Skia.Vulkan.Interop; //Was previously: Uno.UI.Runtime.Skia.Vulkan.Interop
...
internal class VulkanDisplay : IDisposable
{
		...
		VulkanKhrSurface surface, out VkExtent2D swapchainExtent, VulkanDisplay? oldDisplay = null)
	{
		...
		bool supportsIdentityTransform =
			(capabilities.supportedTransforms & VkSurfaceTransformFlagsKHR.VK_SURFACE_TRANSFORM_IDENTITY_BIT_KHR)
				== VkSurfaceTransformFlagsKHR.VK_SURFACE_TRANSFORM_IDENTITY_BIT_KHR;

		bool isRotated =
			(capabilities.currentTransform & VkSurfaceTransformFlagsKHR.VK_SURFACE_TRANSFORM_ROTATE_90_BIT_KHR)
				== VkSurfaceTransformFlagsKHR.VK_SURFACE_TRANSFORM_ROTATE_90_BIT_KHR
			|| (capabilities.currentTransform & VkSurfaceTransformFlagsKHR.VK_SURFACE_TRANSFORM_ROTATE_270_BIT_KHR)
	...
	/// <summary>
	/// Safely recreate the swapchain (no surface recreation).
	/// Call after DeviceWaitIdle when the swapchain is out of date.
	/// </summary>
	public void RecreateSwapchainSafe()
	{
		_context.DeviceApi.DeviceWaitIdle(_context.DeviceHandle);
		DestroyCurrentImageViews();
		_swapchain = CreateSwapchain(_context, _surface!, out var extent, this);
	...
	/// <summary>
	/// Try to acquire the next swapchain image without retrying or recreating.
	/// Returns VK_SUCCESS (0) on success, or the VkResult error code.
	/// </summary>
	public int TryAcquireNextImage()
	{
		_nextImage = 0;
		var result = _context.DeviceApi.AcquireNextImageKHR(
			_context.DeviceHandle,
			_swapchain,
			ulong.MaxValue,
			_semaphorePair.ImageAvailableSemaphore.Handle,
	...
	/// <summary>
	/// Transition the current swapchain image to TRANSFER_DST layout for blit.
	/// Call after TryAcquireNextImage succeeds.
	/// </summary>
	public void PrepareCurrentImageForBlit(VulkanCommandBuffer commandBuffer)
	{
		VulkanMemoryHelper.TransitionLayout(_context, commandBuffer,
			_swapchainImages[_nextImage], VkImageLayout.VK_IMAGE_LAYOUT_UNDEFINED,
			VkAccessFlags.VK_ACCESS_NONE, VkImageLayout.VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
	...
	internal unsafe void EndPresentation(VulkanCommandBuffer commandBuffer)
	{
		...
		_context.DeviceApi.QueuePresentKHR(_context.MainQueueHandle, ref presentInfo)
