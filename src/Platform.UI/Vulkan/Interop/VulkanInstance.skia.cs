...
internal class VulkanInstance : IVulkanInstance
{
	...
	public VulkanInstance(VkInstance handle, VkGetInstanceProcAddressDelegate getProcAddress, string[] enabledExtensions, uint apiVersion)
	{
		Handle = handle;
	...
	{
		using var name = new Utf8Buffer("CodeBrixPlatform"); //Was previously: UnoUI
		using var engineName = new Utf8Buffer("CodeBrixPlatform"); //Was previously: UnoUI

		var gapi = new VulkanGlobalApi(getProcAddress);

		var supportedExtensions = GetSupportedExtensions(gapi);
		// Declare the loader's supported instance version (not a hardcoded 1.1) so that core
		// functions from newer Vulkan versions remain resolvable through vkGet*ProcAddr. Skia
		// milestones newer than the one Uno 6.7.x targets (e.g. the m148-based SkiaSharp used
		// here) load Vulkan 1.2-1.4 core entry points when the device advertises them, and the
		// loader only exposes those when the instance declared a matching apiVersion.
		var apiVersion = Math.Max(gapi.EnumerateInstanceVersion(), VulkanHelpers.MakeVersion(1, 1, 0));
		...
		var instance = new VulkanInstance(pInstance, getProcAddress, enabledExtensions.ToArray(), apiVersion);
