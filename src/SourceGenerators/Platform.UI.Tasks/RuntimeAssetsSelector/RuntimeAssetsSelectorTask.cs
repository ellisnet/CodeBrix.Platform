...
namespace CodeBrix.Platform.UI.Tasks.RuntimeAssetsSelector //Was previously: Uno.UI.Tasks.RuntimeAssetsSelector
{
	/// <summary>
	...
	public class RuntimeAssetsSelectorTask_v0 : Microsoft.Build.Utilities.Task
	{
		...
		[Required]
		public Microsoft.Build.Framework.ITaskItem[] CodeBrixRuntimeEnabledPackage { get; set; } = [];
		...
		public string CodeBrixRuntimeIdentifier { get; set; } = "";

		public string CodeBrixUIRuntimeIdentifier { get; set; } = "";

		public string CodeBrixWinRTRuntimeIdentifier { get; set; } = "";
		...
		public override bool Execute()
		{
			try
			{
				...
				// 1. Packages that are runtime-enabled (e.g, contains codebrix-platform-runtime - which is signified by <CodeBrixRuntimeEnabledPackage Include="PackageName" PackageBasePath="..." /> in package props/targets)
				// 2. Packages that are not runtime-enabled.
				//
				// We also have two modes:
				// 1. Single layer mode (non-null CodeBrixRuntimeIdentifier, expected to be either skia or webassembly).
				// 2. Two layer mode (CodeBrixUIRuntimeIdentifier being skia, while CodeBrixWinRTRuntimeIdentifier expected to be webassembly, android, or ios).
				//
				//
				// For runtime-enabled packages:
				//     Single layer mode:
				//         - Don't alter compile file definitions, we still keep reference to be passed to the compiler.
				//         - Adjust RuntimeCopyLocalItems such that reference binaries are removed and binaries from codebrix-platform-runtime/[skia|wasm] are added.
				//
				//     Two layer mode:
				//         - Adjust RuntimeCopyLocalItems such that WinRT assemblies are added from codebrix-platform-runtime/webassembly or from lib/netX.0-[android|ios] and other assemblies from codebrix-platform-runtime/skia.
				...
				//                 - we should only do that for dlls that reference Uno.UI.dll (use CodeBrix.AssemblyTools to detect that)
				...
				var isSingleLayer = !string.IsNullOrWhiteSpace(CodeBrixRuntimeIdentifier);
				if (isSingleLayer && CodeBrixRuntimeIdentifier is not ("skia" or "webassembly"))
				{
					this.Log.LogError($"The value '{CodeBrixRuntimeIdentifier}' not expected for 'CodeBrixRuntimeIdentifier'");
					return false;
				}

				var isTwoLayer = !isSingleLayer && CodeBrixUIRuntimeIdentifier == "skia";
				if (isTwoLayer && !IsSkiaMobileOrWasmRuntimeIdentifier(CodeBrixWinRTRuntimeIdentifier))
				{
					this.Log.LogError($"The combination of CodeBrixUIRuntimeIdentifier '{CodeBrixUIRuntimeIdentifier}' and CodeBrixWinRTRuntimeIdentifier '{CodeBrixWinRTRuntimeIdentifier}' is not expected");
					return false;
				}
				...
				foreach (var package in CodeBrixRuntimeEnabledPackage ?? Array.Empty<ITaskItem>())
				{
		...
		private string? GetCodeBrixRuntimeDirectory(ITaskItem package)
		{
			var packageBasePath = package.GetMetadata("PackageBasePath");
			string runtimeDirectory = Path.Combine(packageBasePath, "codebrix-platform-runtime");
			if (Directory.Exists(runtimeDirectory))
			{
				return runtimeDirectory;
			}

			runtimeDirectory = Path.Combine(packageBasePath, "..", "codebrix-platform-runtime");
			if (Directory.Exists(runtimeDirectory))
			{
				return runtimeDirectory;
			}
		...
		private string? GetPlatformSpecificDirectoryForRuntimeEnabled(string runtimeDirectory, Version targetFrameworkVersion, bool isTwoLayer)
		{
			...
			if (isTwoLayer)
			{
				...
				if (CodeBrixUIRuntimeIdentifier != "skia")
				{
					throw new Exception($"Unexpected CodeBrixUIRuntimeIdentifier '{CodeBrixUIRuntimeIdentifier}'");
				}

				runtimeIdentifier = CodeBrixUIRuntimeIdentifier;
			}
			else
			{
				// Single layer mode
				runtimeIdentifier = CodeBrixRuntimeIdentifier;
			}
		...
		private bool IsWinRTAssembly(string fileNameWithoutExtension)
			=> fileNameWithoutExtension.ToLower(CultureInfo.InvariantCulture) is "codebrix.platform" or "codebrix.platform.ui.dispatching" or "codebrix.platform.foundation";
		...
		private string GetWinRTAssembly(string runtimeDirectory, string assembly, Version targetFrameworkVersion)
		{
			...
			// <NuGetPackageRoot>/<PackageName>/<PackageVersion>/codebrix-platform-runtime/<TargetFramework>/<RuntimeIdentifier>/<AssemblyName>.dll
			assembly = Path.GetFullPath(assembly);
			// Never null: the asset path layout documented above guarantees enough directory depth
			var codebrixRuntimeTfmDirectory = Path.GetDirectoryName(Path.GetDirectoryName(assembly))!;
			if (CodeBrixWinRTRuntimeIdentifier == "webassembly")
			{
				return Path.GetFullPath(Path.Combine(codebrixRuntimeTfmDirectory, "webassembly", Path.GetFileName(assembly)));
			}

			if (!IsSkiaMobileRuntimeIdentifier(CodeBrixWinRTRuntimeIdentifier))
			{
				throw new Exception($"Unexpected CodeBrixWinRTRuntimeIdentifier '{CodeBrixWinRTRuntimeIdentifier}'");
			}

			var packageRoot = Path.GetDirectoryName(Path.GetDirectoryName(codebrixRuntimeTfmDirectory))!;
			...
			foreach (var dir in Directory.GetDirectories(lib))
			{
				...
				var dashIndex = tfm.IndexOf($"-{CodeBrixWinRTRuntimeIdentifier}", StringComparison.Ordinal);
				if (tfm.StartsWith("net", StringComparison.Ordinal) && dashIndex >= 6 &&
					Version.TryParse(tfm.AsSpan(3, dashIndex - 3), out var currentVersion) &&
			...
			bool isTwoLayer)
		{
			...
			if (GetCodeBrixRuntimeDirectory(package) is not { } runtimeDirectory)
			{
				this.Log.LogMessage($"Cannot find codebrix-platform-runtime in package '{packageIdentity}'.");
				return;
			}
			...
			{
				// This can happen for "legacy convention" (codebrix-platform-runtime/<runtime-identifier>) which is handled by MSBuild logic in ReplaceCodeBrixRuntime
				this.Log.LogMessage("Cannot find platform-specific directory for runtime-enabled package");
				this.Log.LogMessage($"\tThe codebrix-platform-runtime directory: {runtimeDirectory}");
			...
			foreach (var assembly in Directory.EnumerateFiles(platformDirectory, "*.dll"))
			{
				...
				var pdbFile = string.Concat(adjustedAssembly.AsSpan(0, adjustedAssembly.Length - 3), "pdb");
				...
				if (isTwoLayer && IsSkiaMobileRuntimeIdentifier(CodeBrixWinRTRuntimeIdentifier))
				{
			...
			List<ITaskItem> pdbFilesToAdd)
		{
			...
			if (IsSkiaMobileRuntimeIdentifier(CodeBrixWinRTRuntimeIdentifier))
			{
				var runtimeEnabledPackages = CodeBrixRuntimeEnabledPackage.Select(p => p.GetMetadata("Identity")).ToImmutableHashSet(StringComparer.OrdinalIgnoreCase);
				var nugetCacheRoot = NuGetPackageRoot.Replace('\\', '/');
				if (!nugetCacheRoot.EndsWith('/'))
				{
					nugetCacheRoot += "/";
				}
							...
							if (!runtimeEnabledPackages.Contains(packageName))
							{
									...
									targetFramework.Contains("-tvos"))
								{
									...
									if (File.Exists(adjustedPath))
									{
										...
										var pdbFile = string.Concat(fullAdjustedPath.AsSpan(0, fullAdjustedPath.Length - 3), "pdb");
										if (File.Exists(pdbFile))
										{
											pdbFilesToAdd.Add(new TaskItem(
												pdbFile,
