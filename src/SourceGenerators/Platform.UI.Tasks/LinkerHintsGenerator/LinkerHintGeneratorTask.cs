...
namespace CodeBrix.Platform.UI.Tasks.LinkerHintsGenerator //Was previously: Uno.UI.Tasks.LinkerHintsGenerator
{
	...
	public class LinkerHintGeneratorTask_v0 : Microsoft.Build.Utilities.Task
	{
		...
		[Required]
		public string CodeBrixUIPackageBasePath { get; set; } = "";
		...
		private void RunLinker(string outputPath, string features)
		{
			...
			{
				$"--feature CodeBrixBindableMetadata false",
				$"--verbose",
				$"--deterministic",
				$"--trim-mode link",
				$"--action link",
		...
		private Dictionary<string, string> BuildResultingFeaturesList(string resultPath)
		{
			...
			foreach (var asm in assemblies)
			{
				...
				var additionalLinkerHints = asm
					.MainModule
					.GetCustomAttributes()
					.Where(a => a.AttributeType.FullName == "CodeBrix.Platform.Foundation.Diagnostics.CodeAnalysis.AdditionalLinkerHintAttribute");
		...
		private void BuildReferences()
		{
			...
			{
				// Never null: CodeBrixUIPackageBasePath points deep inside the NuGet package cache
				var codebrixUIPackageBasePath = Path.GetDirectoryName(Path.GetDirectoryName(Path.GetDirectoryName(CodeBrixUIPackageBasePath)))!;
				...
				foreach (var referencePath in ReferencePath)
				{
					...
					_referencedAssemblies.Add(RewriteReferencePath(referencePath.ItemSpec, codebrixUIPackageBasePath, CodeBrixRuntimeIdentifier));
				}
