...
namespace CodeBrix.Platform.UI.Tasks.WinAppSDKValidations; //Was previously: Uno.UI.Tasks.WinAppSDKValidations
	...
	public override bool Execute()
	{
		foreach (var project in ReferencedProjects)
		{
			...
			if (targetFrameworkElement is not null)
			{
				...
				if (projectAdditionalProperties.TryGetValue("_IsCodeBrixPlatform", out var isCodeBrixPlatformValue)
					&& bool.TryParse(isCodeBrixPlatformValue, out bool isCodeBrixPlatform)
					&& isCodeBrixPlatform)
				{
					Log.LogError(
						subcategory: null,
