...
namespace CodeBrix.Platform.UI.Tasks.RuntimeAssetsValidator; //Was previously: Uno.UI.Tasks.RuntimeAssetsValidator
/// <summary>
		...
		try
		{
			...
			foreach (var assembly in RuntimeCopyLocalItemsInput ?? [])
			{
				...
				if (
					...
					&& (metadata.ConstructorArguments[0].Value?.ToString()?.Equals("CodeBrixUIRuntimeIdentifier", StringComparison.OrdinalIgnoreCase) ?? false)
					)
				{
					var identifier = metadata.ConstructorArguments[1].Value?.ToString();

					if (!string.IsNullOrEmpty(identifier)
						&& !CodeBrixUIRuntimeIdentifier.Equals(identifier, StringComparison.OrdinalIgnoreCase))
					{
						succeeded = false;

						Log.LogError(
							$"The assembly {assembly.ItemSpec} has a different CodeBrixUIRuntimeIdentifier than the one used to build the project. " +
							$"(Expected: {CodeBrixUIRuntimeIdentifier}, Actual: {identifier})"
						);
					}
				}
			}
