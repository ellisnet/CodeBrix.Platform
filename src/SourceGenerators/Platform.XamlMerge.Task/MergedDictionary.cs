    ...
    public class MergedDictionary
    {
        ...
        private void AddNode(XmlNode node, Dictionary<string, string> xmlnsReplacementDictionary)
        {
            ...
            else
            {
                ...
                if (simpleNodeKey.Length != 0 && nodeKeyToNodeListIndexDictionary.TryGetValue(nodeKey, out int previousNodeIndex))
                {
        ...
        private static void ReplaceNamespacePrefix(XmlNode currentNode, Dictionary<string, string> xmlnsReplacementDictionary)
        {
            ...
            {
                if (xmlnsReplacementDictionary.TryGetValue(currentNode.Prefix, out var replacementPrefix))
                {
                    currentNode.Prefix = replacementPrefix;
                }
            }
        ...
        private static string GetKey(XmlNode node)
        {
            ...
            if (key.Length > 0)
            {
                ...
                {
                    key = string.Concat(node.NamespaceURI.AsSpan(indexOfContractPresent), ":", key);
                }
            }
        ...
        private static string GetStandardNamespace(string name, string value)
        {
            ...
            if (standardNameDictionary.TryGetValue(value, out var standardName))
            {
                return standardName;
            }
            return name;
        ...
        {
            if (nodeKeyToNodeListIndexDictionary.TryGetValue(key, out var nodeIndex))
            {
                nodeListNodesToIgnore.Add(nodeIndex);
            }
