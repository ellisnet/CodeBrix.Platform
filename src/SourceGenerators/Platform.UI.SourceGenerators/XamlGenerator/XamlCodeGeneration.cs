...
using __codebrix::CodeBrix.Platform.Xaml;
...
using CodeBrix.Platform.UI.SourceGenerators.XamlGenerator.ThirdPartyGenerators;
using CodeBrix.Platform.UI.SourceGenerators.XamlGenerator.ThirdPartyGenerators.CommunityToolkitMvvm;

namespace CodeBrix.Platform.UI.SourceGenerators.XamlGenerator //Was previously: Uno.UI.SourceGenerators.XamlGenerator
{
	internal record struct XamlSource(CodeBrix.Platform.Roslyn.MSBuildItem Item, string Link);
	...
	internal partial class XamlCodeGeneration
	{
		...
		internal const string ParseContextPropertyType = "global::CodeBrix.Platform.UI.Xaml.XamlParseContext";
		...
		private readonly CodeBrix.Platform.Roslyn.MSBuildItem[] _resourceFiles;
		private readonly Dictionary<string, string[]> _uiAutomationMappings;
		private readonly string _configuration;
		private readonly bool _isDebug;

		/// <summary>
		/// Should hot reload-related calls be generated? By default this is true iff building in debug, but it can be forced to always true or false using the "CodeBrixForceHotReloadCodeGen" project flag.
		/// </summary>
		...
		private readonly bool _isCodeBrixHead;
		...
		private bool IsCodeBrixAssembly
			=> _defaultNamespace == "CodeBrix.Platform.UI";

		private bool IsCodeBrixFluentAssembly
			=> _defaultNamespace == "CodeBrix.Platform.UI.FluentTheme" || _defaultNamespace.StartsWith("CodeBrix.Platform.UI.FluentTheme.v", StringComparison.Ordinal);
		...
		public XamlCodeGeneration(GeneratorExecutionContext context)
		{
			...
			// Add <CodeBrixUISourceGeneratorDebuggerBreak>True</CodeBrixUISourceGeneratorDebuggerBreak> to your project

			if (!Helpers.DesignTimeHelper.IsDesignTime(context)
				&& (context.GetMSBuildPropertyValue("CodeBrixUISourceGeneratorDebuggerBreak")?.Equals("True", StringComparison.OrdinalIgnoreCase) ?? false))
			{
				Debugger.Launch();
			}
			...
			var pageItems = GetWinUIItems("Page").Concat(GetWinUIItems("CodeBrixPage"));
			var applicationDefinitionItems = GetWinUIItems("ApplicationDefinition").Concat(GetWinUIItems("CodeBrixApplicationDefinition"));
			...
			if (bool.TryParse(context.GetMSBuildPropertyValue("CodeBrixXamlLazyVisualStateManagerEnabled"), out var isLazyVisualStateManagerEnabled))
			{
				_isLazyVisualStateManagerEnabled = isLazyVisualStateManagerEnabled;
			}

			if (bool.TryParse(context.GetMSBuildPropertyValue("CodeBrixXamlResourcesTrimming"), out var xamlResourcesTrimming))
			{
				_xamlResourcesTrimming = xamlResourcesTrimming;
			}

			if (bool.TryParse(context.GetMSBuildPropertyValue("IsCodeBrixHead"), out var isCodeBrixHead))
			{
				_isCodeBrixHead = isCodeBrixHead;
			}

			if (bool.TryParse(context.GetMSBuildPropertyValue("CodeBrixForceHotReloadCodeGen"), out var isHotReloadEnabled))
			{
			...
			if (!bool.TryParse(context.GetMSBuildPropertyValue("CodeBrixGenerateXamlSourcesProvider"), out _generateXamlSourcesProvider))
			{
				_generateXamlSourcesProvider = _isHotReloadEnabled; // Default to the presence of Hot Reload feature
			}

			if (!bool.TryParse(context.GetMSBuildPropertyValue("CodeBrixEnableXamlFuzzyMatching"), out _enableFuzzyMatching))
			{
				_enableFuzzyMatching = false;
			}

			if (!bool.TryParse(context.GetMSBuildPropertyValue("CodeBrixDisableBindableTypeProvidersGeneration"), out _disableBindableTypeProvidersGeneration))
			{
		...
		private static bool IsWinUIItem(CodeBrix.Platform.Roslyn.MSBuildItem item)
			=> item.GetMetadataValue("XamlRuntime") is { } xamlRuntime
				? xamlRuntime == "WinUI" || string.IsNullOrWhiteSpace(xamlRuntime)
				: true;

		private IEnumerable<CodeBrix.Platform.Roslyn.MSBuildItem> GetWinUIItems(string name)
		...
		/// </summary>
		private string GetSourceLink(CodeBrix.Platform.Roslyn.MSBuildItem projectItemInstance)
		{
				...
				var csharpFiles = (Debugger.IsAttached
					...
					.Select<XamlFileDefinition, (XamlFileDefinition definition, SourceText? code, IEnumerable<XamlGenerationException> errors)>(file =>
					{
						var generator = new XamlFileGenerator(
							...
							isCodeBrixAssembly: IsCodeBrixAssembly,
							isCodeBrixFluentAssembly: IsCodeBrixFluentAssembly,
		...
		private void TryGenerateCodeBrixResourcesKeyAttribute(ResourceDetailsCollection resourceDetailsCollection)
		{
			...
			_generatorContext.AddSource(
				...
				$"""
				// <auto-generated />
				[assembly: global::System.Reflection.AssemblyMetadata("CodeBrixHasLocalizationResources", "{hasResources.ToString(CultureInfo.InvariantCulture)}")]
				""");
		}
		...
		private SourceText GenerateGlobalResources(XamlFileDefinition[] xamlFiles, XamlGlobalStaticResourcesMap map, INamedTypeSymbol[] ambientGlobalResources)
		{
			...
			using (writer.BlockInvariant("namespace {0}", _defaultNamespace))
			{
				...
				writer.AppendLineIndented($"[global::System.CodeDom.Compiler.GeneratedCode(\"CodeBrix.Platform.UI.SourceGenerators.XamlGenerator.XamlCodeGeneration\", \"{GeneratorVersion}\")]");
				...
				using (writer.BlockInvariant("public sealed partial class GlobalStaticResources"))
				{
					...
					using (writer.BlockInvariant("public static void Initialize()"))
					{
						...
						{
							using (IsCodeBrixAssembly ? writer.BlockInvariant("using (ResourceResolver.WriteInitiateGlobalStaticResourcesEventActivity())") : null)
							{
								...
								if (IsCodeBrixAssembly && xamlFiles.Length > 0)
								{
					...
					using (writer.BlockInvariant("public static void RegisterResourceDictionariesBySource()"))
					{
						using (writer.BlockInvariant("if(!_dictionariesRegistered)"))
						{
							...
							if (!IsCodeBrixAssembly && !IsCodeBrixFluentAssembly)
							{
							...
							else if (xamlFiles.Length != 0 && IsCodeBrixFluentAssembly)
							{
							...
							void RegisterForXamlFile(XamlFileDefinition file, string url)
							{
								if (file != null)
								{
									writer.AppendLineIndented($"global::CodeBrix.Platform.UI.ResourceResolver.RegisterResourceDictionaryBySource(uri: \"ms-appx:///{url}\", context: {ParseContextPropertyName}, dictionary: () => {file.UniqueID}_ResourceDictionary);");
								}
							}
						}
					}
					...
					using (writer.BlockInvariant("internal static void RegisterResourceDictionariesBySourceLocal()"))
					{
						foreach (var file in xamlFiles.Where(IsResourceDictionary))
						{
							...
							writer.AppendLineIndented($"global::CodeBrix.Platform.UI.ResourceResolver.RegisterResourceDictionaryBySource(uri: \"{XamlFilePathHelper.LocalResourcePrefix}{map.GetSourceLink(file)}\", context: null, dictionary: () => {file.UniqueID}_ResourceDictionary, {filePath});");
							// Local resources can also be found through the ms-appx:/// prefix
							writer.AppendLineIndented($"global::CodeBrix.Platform.UI.ResourceResolver.RegisterResourceDictionaryBySource(uri: \"{XamlFilePathHelper.AppXIdentifier}{map.GetSourceLink(file)}\", context: null, dictionary: () => {file.UniqueID}_ResourceDictionary);");
						}
					}

					if (IsCodeBrixAssembly)
					{
