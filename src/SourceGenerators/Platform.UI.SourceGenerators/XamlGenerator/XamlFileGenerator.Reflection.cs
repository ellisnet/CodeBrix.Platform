...
namespace CodeBrix.Platform.UI.SourceGenerators.XamlGenerator //Was previously: Uno.UI.SourceGenerators.XamlGenerator
{
	internal partial class XamlFileGenerator
	{
		...
		private void InitCaches()
		{
			...
			_clrNamespaces = _knownNamespaces.PlatformGetValueOrDefault(defaultXmlNamespace, Array.Empty<string>());
		}
		...
		private INamedTypeSymbol? SourceFindTypeByXamlType(XamlType type)
		{
			if (type != null)
			{
				...
				var clrNamespaces = _knownNamespaces.PlatformGetValueOrDefault(trimmedNamespace, Array.Empty<string>());
		...
		private INamedTypeSymbol? SourceFindType(string name)
		{
			...
			else if (name.Contains(":"))
			{
				...
				else if (nsName.Equals("CodeBrix.Platform.UI.Controls.Legacy") &&
